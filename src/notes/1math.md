...
- $\int_{a}^{b}=b-a=L$，其中$L$是$[a,b]$的长度.
- 无论$a,b,c$大小如何，总有$\int_{a}^{b}f(x)dx=\int_{a}^{c}f(x)dx+\int_{c}^{b}f(x)dx$.
- 设$k_1$，$k_2$为常数，则$\int_{a}^{b}k_1f(x)\pm k_2g(x)dx=k_1\int_{a}^{b}f(x)dx\pm k_2\int_{a}^{b}g(x)dx$.
