<template><div><h3 id="操作系统的功能" tabindex="-1"><a class="header-anchor" href="#操作系统的功能" aria-hidden="true">#</a> 操作系统的功能</h3>
...
<!-- more -->
<h3 id="什么是用户态和内核态" tabindex="-1"><a class="header-anchor" href="#什么是用户态和内核态" aria-hidden="true">#</a> 什么是用户态和内核态？</h3>
<ul>
<li>用户态：又称为目态，用户态运行的进程可以直接读取用户程序的数据，拥有较低的权限。当应用程序需要执行某些特权指令的时候就会发生系统调用，让用户态变成内核态。</li>
<li>内核态：又称为管态，内核态运行的进程集合可以访问计算机的任何资源，拥有非常高的权限，当操作系统接收到进程的系统调用请求时，就会从用户态切换为内核态，执行响应的系统调用，并将结果返回给进程，最后再从内核态切换为用户态。</li>
</ul>
<p>用户态切换到内核态的三种方式：</p>
<ol>
<li>系统调用：用户态进程主动要求切换到内核态的一种方式；</li>
<li>中断：当外围设备完成用户请求操作之后，会向CPU发出响应的中断信号，CPU响应的时候切换到核心态。比如硬盘读写操作完成，系统会切换到硬盘读写的中断处理程序中执行后续操作等。</li>
...
<h3 id="什么是进程和线程" tabindex="-1"><a class="header-anchor" href="#什么是进程和线程" aria-hidden="true">#</a> 什么是进程和线程？</h3>
<ul>
<li>进程：值得是计算机中正在运行的程序实例，是操作系统中拥有资源的基本单位。</li>
<li>线程：引入线程之后，CPU调度的最小单位变成线程，线程又称为轻量级进程。</li>
</ul>
<p>对于Java来说，一个进程能拥有很多个线程，多个线程共享进程的堆和方法区（JDK1.8之后的元空间），但是每个线程都有自己的程序计数器，虚拟机栈和本地方法栈（线程私有）。</p>
<h3 id="有了进程为什么还需要线程" tabindex="-1"><a class="header-anchor" href="#有了进程为什么还需要线程" aria-hidden="true">#</a> 有了进程为什么还需要线程?</h3>
<ul>
<li>进程切换的成本大于线程切换。</li>
<li>线程更加轻量，一个进程能创建多个线程。</li>
<li>多个线程能并发处理不同的任务，有效利用CPU资源。而进程只能在一个时间干一件事，如果在执行过程中遇到阻塞就会挂起直到结果返回。</li>
<li>同一个进程的线程共享内存和文件，因此线程之间的相互通信无需调用内核。</li>
</ul>
<h3 id="线程间的同步的方式有哪些" tabindex="-1"><a class="header-anchor" href="#线程间的同步的方式有哪些" aria-hidden="true">#</a> 线程间的同步的方式有哪些？</h3>
<ul>
<li>
<p><strong>互斥锁(Mutex)</strong>：采用互斥对象机制，只有拥有互斥对象的线程才有访问公共资源的权限。因为互斥对象只有一个，所以可以保证公共资源不会被多个线程同时访问。比如 Java 中的 <code v-pre>synchronized</code> 关键词和各种 <code v-pre>Lock</code> 都是这种机制。</p>
</li>
<li>
...
<li>
<p><strong>屏障（Barrier）</strong>：屏障是一种同步原语，用于等待多个线程到达某个点再一起继续执行。当一个线程到达屏障时，它会停止执行并等待其他线程到达屏障，直到所有线程都到达屏障后，它们才会一起继续执行。比如 Java 中的 <code v-pre>CyclicBarrier</code> 是这种机制。</p>
</li>
<li>
<p><strong>事件(Event)</strong> :Wait/Notify：通过通知操作的方式来保持多线程同步，还可以方便的实现多线程优先级的比较操作。</p>
</li>
</ul>
<h3 id="pcb-是什么-包含哪些信息" tabindex="-1"><a class="header-anchor" href="#pcb-是什么-包含哪些信息" aria-hidden="true">#</a> PCB 是什么？包含哪些信息？</h3>
...
<h3 id="进程间的通信方式有哪些" tabindex="-1"><a class="header-anchor" href="#进程间的通信方式有哪些" aria-hidden="true">#</a> 进程间的通信方式有哪些？</h3>
<ul>
<li>
<p><strong>管道/匿名管道(Pipes)</strong>：用于具有亲缘关系的父子进程间或者兄弟进程之间的通信。</p>
</li>
<li>
...
<li>
<p><strong>信号量(Semaphores)</strong>：信号量是一个计数器，用于多进程对共享数据的访问，信号量的意图在于进程间同步。这种通信方式主要用于解决与同步相关的问题并避免竞争条件。</p>
</li>
<li>
<p><strong>共享内存(Shared memory)</strong>：使得多个进程可以访问同一块内存空间，不同进程可以及时看到对方进程中对共享内存中数据的更新。这种方式需要依靠某种同步操作，如互斥锁和信号量等。可以说这是最有用的进程间通信方式。</p>
</li>
<li>
...
<h3 id="什么是僵尸进程和孤儿进程" tabindex="-1"><a class="header-anchor" href="#什么是僵尸进程和孤儿进程" aria-hidden="true">#</a> 什么是僵尸进程和孤儿进程？</h3>
<ul>
<li>
...
<h3 id="什么是内存碎片" tabindex="-1"><a class="header-anchor" href="#什么是内存碎片" aria-hidden="true">#</a> 什么是内存碎片？</h3>
<ul>
<li>
<p><strong>内部内存碎片(Internal Memory Fragmentation，简称为内存碎片)</strong>：<strong>已经分配给进程使用但未被使用的内存</strong>。导致内部内存碎片的主要原因是，当采用固定比例比如 2 的幂次方进行内存分配时，进程所分配的内存可能会比其实际所需要的大。举个例子，一个进程只需要 65 字节的内存，但为其分配了 128（2^7） 大小的内存，那 63 字节的内存就成为了内部内存碎片。（固定分区分配）</p>
</li>
<li>
<p><strong>外部内存碎片(External Memory Fragmentation，简称为外部碎片)</strong>：由于未分配的连续内存区域太小，以至于不能满足任意进程所需要的内存分配请求，这些小片段且不连续的内存空间被称为外部碎片。也就是说，外部内存碎片指的是那些并为分配给进程但又不能使用的内存。我们后面介绍的分段机制就会导致外部内存碎片。<strong>没有分配的内存多出的碎片</strong>。（动态分区分配）</p>
</li>
</ul>
<h3 id="分段分页" tabindex="-1"><a class="header-anchor" href="#分段分页" aria-hidden="true">#</a> 分段分页</h3>
<p>...</p>
<h2 id="文件系统" tabindex="-1"><a class="header-anchor" href="#文件系统" aria-hidden="true">#</a> 文件系统</h2>
<h3 id="硬链接和软链接有什么区别" tabindex="-1"><a class="header-anchor" href="#硬链接和软链接有什么区别" aria-hidden="true">#</a> 硬链接和软链接有什么区别？</h3>
...
<p>（5）删除一个硬链接文件并不影响其他有相同inode 号的文件。</p>
<p><strong>2、软链接（Symbolic Link 或 Symlink）</strong></p>
<p>又被叫为符号链接（symbolic  Link），它包含了到原文件的路径信息。</p>
<p>文件被删除，则相关软连接被称为死链接（即 dangling link，若被指向路径文件被重新创建，死链接可恢复为正常的软链接）。（快捷方式）</p>
<p><strong>本质区别</strong></p>
<p>硬链接：本质是同一个文件</p>
<p>软链接：本质不是同一个文件</p>
<h3 id="常见的磁盘调度算法有哪些" tabindex="-1"><a class="header-anchor" href="#常见的磁盘调度算法有哪些" aria-hidden="true">#</a> 常见的磁盘调度算法有哪些？</h3>
<ul>
<li>
