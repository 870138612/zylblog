<template><div><p>消息队列是一种用于在应用程序之间传递消息的通信方式，消息队列允许应用程序异步的发送和接收消息，并且不需要直接连接到对方。</p>
<p>可以把消息队列看作是一个存放消息的容器，需要使用消息的时候，直接从容器中取出消息供自己使用即可。由于队列 Queue 是一种先进先出的数据结构，所以消费消息时也是按照顺序来消费的。</p>
<h2 id="消息队列有什么用" tabindex="-1"><a class="header-anchor" href="#消息队列有什么用" aria-hidden="true">#</a> 消息队列有什么用？</h2>
<h3 id="通过异步处理提高系统性能-减少响应所需时间" tabindex="-1"><a class="header-anchor" href="#通过异步处理提高系统性能-减少响应所需时间" aria-hidden="true">#</a> 通过异步处理提高系统性能（减少响应所需时间）</h3>
<p>项目中订单取消之后的解锁库存操作可以通过消息队列实现，订单取消之后发送一个库存释放任务到消息队列中，由库存模块释放监听器完成库存解锁和释放，订单模块不需要等待库存释放完成才返回。</p>
<h3 id="削峰-限流" tabindex="-1"><a class="header-anchor" href="#削峰-限流" aria-hidden="true">#</a> 削峰/限流</h3>
<p>秒杀场景中，开始秒杀时创建秒杀订单具有很大的峰值，通过消息队列减少同时创建订单的任务数，创建订单任务先放入消息队列中，再由数据库创建订单。</p>
<h3 id="降低系统耦合性" tabindex="-1"><a class="header-anchor" href="#降低系统耦合性" aria-hidden="true">#</a> 降低系统耦合性</h3>
<p>订单模块负责订单取消，库存模块负责库存解锁释放，两者通过消息队列实现通信，降低耦合度。如果有新的消费者上线，只需要将消费者关联到这个消息队列中，原有系统不会改变。</p>
<h3 id="实现分布式事务" tabindex="-1"><a class="header-anchor" href="#实现分布式事务" aria-hidden="true">#</a> 实现分布式事务</h3>
<p>分布式事务的解决办法之一就是MQ事务。</p>
<p>☀️详见<a href="https://ylzhong.top/middleware/4distributedtransaction.html" target="_blank" rel="noopener noreferrer">分布式事务<ExternalLinkIcon/></a></p>
<h2 id="使用消息队列会带来哪些问题" tabindex="-1"><a class="header-anchor" href="#使用消息队列会带来哪些问题" aria-hidden="true">#</a> 使用消息队列会带来哪些问题？</h2>
<ul>
<li><strong>系统可用性降低</strong>：需要额外考虑消息队列服务出错的处理办法。</li>
<li><strong>系统复杂度提高</strong>：加入消息队列之后，需要考虑消息重复消费，消息丢失，保证消息传递顺序等问题。</li>
<li><strong>一致性问题</strong>：如果消息没有被正确的消费，就会带来一致性问题。</li>
</ul>
<h2 id="amqp是什么" tabindex="-1"><a class="header-anchor" href="#amqp是什么" aria-hidden="true">#</a> AMQP是什么？</h2>
<p>AMQP，即 Advanced Message Queuing Protocol，一个提供统一消息服务的应用层标准 <strong>高级消息队列协议</strong>（二进制应用层协议），是应用层协议的一个开放标准，为面向消息的中间件设计，兼容 JMS（JAVA Message Service，java 消息服务）。基于此协议的客户端与消息中间件可传递消息，并不受客户端/中间件同产品，不同的开发语言等条件的限制。AMQP 天然具有跨平台、跨语言特性。</p>
<h2 id="rpc和消息队列的区别" tabindex="-1"><a class="header-anchor" href="#rpc和消息队列的区别" aria-hidden="true">#</a> RPC和消息队列的区别</h2>
<p>RPC和消息队列都是分布式系统中重要的组件之一。</p>
<ul>
<li>
<p><strong>从用途来看</strong>：RPC主要解决两个服务的远程通信问题，不需要了解底层网络的通信机制。通过 RPC 可以帮助我们调用远程计算机上某个服务的方法，这个过程就像调用本地方法一样简单。消息队列主要用来降低系统耦合性、实现任务异步、有效地进行流量削峰。</p>
</li>
<li>
...
<h2 id="常见的消息队列" tabindex="-1"><a class="header-anchor" href="#常见的消息队列" aria-hidden="true">#</a> 常见的消息队列</h2>
<h3 id="kafka" tabindex="-1"><a class="header-anchor" href="#kafka" aria-hidden="true">#</a> Kafka</h3>
<blockquote>
<p>项目中使用的消息队列是RabbitMQ，Kafka仅仅做介绍。</p>
</blockquote>
<p><img src="/markdown/image-20230616154303334.png" alt="image-20230616154303334"></p>
<p>Kafka 是 LinkedIn 开源的一个分布式流式处理平台，已经成为 Apache 顶级项目，早期被用来用于处理海量的日志，后面才慢慢发展成了一款功能全面的高性能消息队列。</p>
<p>具有三个关键功能：</p>
<ul>
<li><strong>消息队列</strong>：发布和订阅消息流，功能类似消息队列。</li>
<li><strong>容错的持久方式存储记录消息流</strong>：Kafka会把消息持久化到磁盘，有效避免了消息丢失的风险。</li>
<li><strong>流式处理平台</strong>：在消息发布的时候进行处理，Kafka提供了一个完整的流式处理类库。</li>
</ul>
<h3 id="rabbitmq" tabindex="-1"><a class="header-anchor" href="#rabbitmq" aria-hidden="true">#</a> RabbitMQ</h3>
<p><img src="/markdown/image-20230616154739983.png" alt="image-20230616154739983"></p>
...
<h3 id="rabbitmq和kafka区别" tabindex="-1"><a class="header-anchor" href="#rabbitmq和kafka区别" aria-hidden="true">#</a> RabbitMQ和Kafka区别</h3>
<ul>
<li>
<p>RabbitMQ 在吞吐量方面虽然稍逊于 Kafka，但是由于它基于 Erlang 开发，所以并发能力很强，性能极其好，延时很低，达到微秒级。但是也因为 RabbitMQ 基于 Erlang 开发，所以国内很少有公司有实力做 Erlang 源码级别的研究和定制。如果业务场景对并发量要求不是太高（十万级、百万级），那这几种消息队列中，RabbitMQ 或许是你的首选。</p>
</li>
<li>
...
<h1 id="rabbitmq-1" tabindex="-1"><a class="header-anchor" href="#rabbitmq-1" aria-hidden="true">#</a> RabbitMQ</h1>
<h2 id="介绍下rabbitmq" tabindex="-1"><a class="header-anchor" href="#介绍下rabbitmq" aria-hidden="true">#</a> 介绍下RabbitMQ</h2>
<p>RabbitMQ 是一个在 AMQP（Advanced Message Queuing Protocol ）基础上实现的，可复用的企业消息系统。它可以用于大型软件系统各个模块之间的高效通信，支持高并发，支持可扩展。</p>
...
<li><strong>跨语言</strong>：RabbitMQ 几乎支持所有常用语言，比如 Java、 Python、 Ruby、 PHP、 C#、 JavaScript 等。</li>
...
<ul>
<li><strong>Producer(生产者)</strong> :生产消息的一方;</li>
...
<h3 id="queue-消息队列" tabindex="-1"><a class="header-anchor" href="#queue-消息队列" aria-hidden="true">#</a> Queue（消息队列）</h3>
<p>Queue（消息队列）用来保存消息直到发送给消费者。是消息的容器也是终点，一个消息可以投入一个或者多个队列中。</p>
<p><strong>RabbitMQ</strong> 中消息只能存储在 <strong>队列</strong> 中，这一点和 <strong>Kafka</strong> 相反。Kafka 将消息存储在 <strong>topic（主题）</strong> 这个逻辑层面，而相对应的队列逻辑只是 topic 实际存储文件中的位移标识。</p>
<blockquote>
<p>事件被组织并持久地存储在Topic中，Topic类似于文件系统中的文件夹，事件就是该文件夹中的文件。Kafka中的Topic始终是多生产者和多订阅者：一个Topic可以有零个、一个或多个生产者向其写入事件，也可以有零个、一个或多个消费者订阅这些事件。Topic中的事件可以根据需要随时读取，与传统的消息中间件不同，事件在使用后不会被删除，相反，可以通过配置来定义Kafka中每个Topic应该保留事件的时间，超过该事件后旧事件将被丢弃。Kafka的性能在数据大小方面实际上是恒定的，因此长时间存储数据是非常好的。</p>
</blockquote>
<p>多个消费者可以订阅同一个队列，这时队列中的消息会被平均分摊给多个消费者进行处理，而不是每个消费者都收到所有的消息并处理，这样可以避免重复消费。</p>
<h3 id="broker-消息中间件的服务节点" tabindex="-1"><a class="header-anchor" href="#broker-消息中间件的服务节点" aria-hidden="true">#</a> Broker（消息中间件的服务节点）</h3>
...
<h4 id="_3、topic" tabindex="-1"><a class="header-anchor" href="#_3、topic" aria-hidden="true">#</a> 3、topic</h4>
<p>topic 类型的交换器在匹配规则上进行了扩展，它与 direct 类型的交换器相似，也是将消息路由到 BindingKey 和 RoutingKey 相匹配的队列中，但这里的匹配规则有些不同，它约定：</p>
<ul>
<li>
<p>RoutingKey 为一个点号<code v-pre>.</code>分隔的字符串（被点号<code v-pre>.</code>分隔开的每一段独立的字符串称为一个单词），如 <code v-pre>com.rabbitmq.client</code>、<code v-pre>java.util.concurrent</code>、<code v-pre>com.hidden.client</code>;</p>
</li>
<li>
<p>BindingKey 和 RoutingKey 一样也是点号<code v-pre>.</code>分隔的字符串；</p>
</li>
<li>
<p>BindingKey 中可以存在两种特殊字符串<code v-pre>*</code>和<code v-pre>#</code>，用于做模糊匹配，其中<code v-pre>*</code>用于匹配一个单词，<code v-pre>#</code>用于匹配零个或者多个单词。</p>
</li>
</ul>
<p>例如BindKey为<code v-pre>#.com</code>，则所有以<code v-pre>com</code>结尾的路由键消息都会匹配，<code v-pre>*.*.com</code>，则路由键必须有三个单词，且最后一个单词是<code v-pre>com</code>才会匹配，例如<code v-pre>yl.zhong.com</code>。</p>
<p><img src="/markdown/image-20230616171738195.png" alt="image-20230616171738195"></p>
<h4 id="_4、headers-不推荐" tabindex="-1"><a class="header-anchor" href="#_4、headers-不推荐" aria-hidden="true">#</a> 4、headers(不推荐)</h4>
<p>headers 类型的交换器不依赖于路由键的匹配规则来路由消息，而是根据发送的消息内容中的 headers 属性进行匹配。</p>
<h2 id="说说-broker-服务节点、queue-队列、exchange-交换器" tabindex="-1"><a class="header-anchor" href="#说说-broker-服务节点、queue-队列、exchange-交换器" aria-hidden="true">#</a> 说说 Broker 服务节点、Queue 队列、Exchange 交换器？</h2>
