<template><div><p>一个最基本的分布式锁需要满足：</p>
<ul>
...
<!-- more -->
<h2 id="基于-redis-实现分布式锁" tabindex="-1"><a class="header-anchor" href="#基于-redis-实现分布式锁" aria-hidden="true">#</a> 基于 Redis 实现分布式锁</h2>
<h3 id="如何基于-redis-实现一个最简易的分布式锁如何基于-redis-实现一个最简易的分布式锁" tabindex="-1"><a class="header-anchor" href="#如何基于-redis-实现一个最简易的分布式锁如何基于-redis-实现一个最简易的分布式锁" aria-hidden="true">#</a> 如何基于-redis-实现一个最简易的分布式锁如何基于 Redis 实现一个最简易的分布式锁？</h3>
<p>在 Redis 中， <code v-pre>SETNX</code> 命令是可以帮助我们实现互斥。<code v-pre>SETNX</code> 即 <strong>SET</strong> if <strong>N</strong>ot e<strong>X</strong>ists (对应 Java 中的 <code v-pre>setIfAbsent</code> 方法)，如果 key 不存在的话，才会设置 key 的值。如果 key 已经存在， <code v-pre>SETNX</code> 啥也不做。</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token operator">></span> SETNX lockKey uniqueValue
<span class="token punctuation">(</span>integer<span class="token punctuation">)</span> <span class="token number">1</span>
<span class="token operator">></span> SETNX lockKey uniqueValue
<span class="token punctuation">(</span>integer<span class="token punctuation">)</span> <span class="token number">0</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>释放锁的话，直接通过 <code v-pre>DEL</code> 命令删除对应的 key 即可。</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token operator">></span> DEL lockKey
<span class="token punctuation">(</span>integer<span class="token punctuation">)</span> <span class="token number">1</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>为了防止误删到其他的锁，建议使用 Lua 脚本通过 key 对应的 value（唯一值）来判断。</p>
<p>选用 Lua 脚本是为了保证解锁操作的原子性。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 释放锁时，先比较锁对应的 value 值是否相等，避免锁的误释放</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>+这是一种最简易的 Redis 分布式锁实现，实现方式比较简单，性能也很高效。不过，这种方式实现分布式锁存在一些问题。例如引用程序释放锁的逻辑挂掉，可能会导致锁无法被释放，进而造成共享资源无法再被其他线程/进程访问。</p>
<h3 id="如何防止释放锁逻辑失效导致的锁无法释放" tabindex="-1"><a class="header-anchor" href="#如何防止释放锁逻辑失效导致的锁无法释放" aria-hidden="true">#</a> 如何防止释放锁逻辑失效导致的锁无法释放？</h3>
<p>为了避免锁无法被释放，可以给锁添加一个过期时间。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token number">127.0</span><span class="token number">.0</span><span class="token number">.1</span><span class="token operator">:</span><span class="token number">6379</span><span class="token operator">></span> <span class="token constant">SET</span> lockKey uniqueValue <span class="token constant">EX</span> <span class="token number">3</span> <span class="token constant">NX</span>
<span class="token constant">OK</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><ul>
<li>
<p><strong>lockKey</strong>：加锁的锁名；</p>
</li>
<li>
<p><strong>uniqueValue</strong>：能够唯一标示锁的随机字符串；</p>
</li>
<li>
<p><strong>NX</strong>：只有当 lockKey 对应的 key 值不存在的时候才能 SET 成功；</p>
</li>
<li>
<p><strong>EX</strong>：过期时间设置（秒为单位）EX 3 标示这个锁有一个 3 秒的自动过期时间。与 EX 对应的是 PX（毫秒为单位），这两个都是过期时间设置。</p>
</li>
</ul>
<p><strong>一定要保证设置指定key的值和过期时间是一个原子操作</strong>，否则依然可能会出现锁无法被释放的问题。</p>
<p>如果操作共享资源的时间大于过期时间，则会导致锁提前释放，分布锁直接失效。设置时间过长，解锁逻辑挂了又会影响性能。</p>
<h3 id="如何实现锁的优雅续期" tabindex="-1"><a class="header-anchor" href="#如何实现锁的优雅续期" aria-hidden="true">#</a> 如何实现锁的优雅续期？</h3>
...
<p>看门狗名字的由来于 <code v-pre>getLockWatchdogTimeout()</code> 方法，这个方法返回的是看门狗给锁续期的过期时间，默认为 30 秒。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//默认 30秒，支持修改</span>
...
<p>Watch Dog 通过调用 <code v-pre>renewExpirationAsync()</code> 方法实现锁的异步续期</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>以 Redisson 的分布式可重入锁 <code v-pre>RLock</code> 为例来说明如何使用 Redisson 实现分布式锁：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 1.获取指定的分布式锁对象</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// 手动给锁设置过期时间，不具备 Watch Dog 自动续期机制</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="如何实现可重入锁" tabindex="-1"><a class="header-anchor" href="#如何实现可重入锁" aria-hidden="true">#</a> 如何实现可重入锁？</h3>
<p>所谓可重入锁指的是在一个线程中可以多次获取同一把锁，比如一个线程在执行一个带锁的方法，该方法中又调用了另一个需要相同锁的方法，则该线程可以直接执行调用的方法即可重入 ，而无需重新获得锁。像 Java 中的 <code v-pre>synchronized</code> 和 <code v-pre>ReentrantLock</code> 都属于可重入锁。</p>
<p><strong>不可重入的分布式锁基本可以满足绝大部分业务场景了，一些特殊的场景可能会需要使用可重入的分布式锁。</strong></p>
<p>我们不需要自己手动实现，推荐使用我们上面提到的 <strong>Redisson</strong> ，其内置了多种类型的锁比如可重入锁（Reentrant Lock）、自旋锁（Spin Lock）、公平锁（Fair Lock）、多重锁（MultiLock）、 红锁（RedLock）、 读写锁（ReadWriteLock）。</p>
<h3 id="redis-如何解决集群情况下分布式锁的可靠性" tabindex="-1"><a class="header-anchor" href="#redis-如何解决集群情况下分布式锁的可靠性" aria-hidden="true">#</a> Redis 如何解决集群情况下分布式锁的可靠性？</h3>
...
<p>针对这个问题，Redis 之父 antirez 设计了<strong>Redlock</strong> 算法来解决。</p>
<p><strong>Redlock 算法的思想是让客户端向 Redis 集群中的多个独立的 Redis 实例依次请求申请加锁</strong>，如果客户端能够和半数以上的实例成功地完成加锁操作，就认为客户端成功地获得分布式锁，否则加锁失败。即使部分 Redis 节点出现问题，只要保证 Redis 集群中有半数以上的 Redis 节点可用，分布式锁服务就是正常的。</p>
<p>Redlock 是直接操作 Redis 节点的，并不是通过 Redis 集群操作的，这样才可以避免 Redis 集群主从切换导致的锁丢失问题。</p>
<h2 id="基于-zookeeper-实现分布式锁" tabindex="-1"><a class="header-anchor" href="#基于-zookeeper-实现分布式锁" aria-hidden="true">#</a> 基于 ZooKeeper 实现分布式锁</h2>
<p>Redis 实现分布式锁性能较高，ZooKeeper 实现分布式锁可靠性更高。</p>
<p>ZooKeeper 分布式锁是基于 <strong>临时顺序节点</strong> 和 <strong>Watcher（事件监听器）</strong> 实现的。</p>
<p>类似AQS的实现方式。</p>
<p>获取锁：</p>
<ol>
<li>首先我们要有一个持久节点<code v-pre>/locks</code>，客户端获取锁就是在<code v-pre>locks</code>下创建临时顺序节点。</li>
<li>假设客户端 1 创建了<code v-pre>/locks/lock1</code>节点，创建成功之后，会判断 <code v-pre>lock1</code>是否是 <code v-pre>/locks</code> 下最小的子节点。</li>
<li>如果 <code v-pre>lock1</code>是最小的子节点，则获取锁成功。否则，获取锁失败。</li>
<li>如果获取锁失败，则说明有其他的客户端已经成功获取锁。<strong>客户端 1 并不会不停地循环去尝试加锁，而是在前一个节点比如<code v-pre>/locks/lock0</code>上注册一个事件监听器</strong>。这个监听器的作用是当前一个节点释放锁之后通知客户端 1（避免无效自旋），这样客户端 1 就加锁成功了。</li>
</ol>
<p>释放锁：</p>
<ol>
<li>成功获取锁的客户端在执行完业务流程之后，会将对应的子节点删除。</li>
<li>成功获取锁的客户端在出现故障之后，对应的子节点由于是临时顺序节点，也会被自动删除，避免了锁无法被释放。</li>
<li>前面说的事件监听器其实监听的就是这个子节点删除事件，子节点删除就意味着锁被释放。</li>
</ol>
<p><img src="/markdown/image-20230607195001297.png" alt="image-20230607195001297"></p>
<h3 id="为什么要用临时顺序节点" tabindex="-1"><a class="header-anchor" href="#为什么要用临时顺序节点" aria-hidden="true">#</a> 为什么要用临时顺序节点？</h3>
<p>每个数据节点在 ZooKeeper 中被称为 <strong>znode</strong>，它是 ZooKeeper 中数据的最小单元。</p>
<p>通常是将 znode 分为 4 大类：</p>
<ul>
...
<li><strong>持久顺序（PERSISTENT_SEQUENTIAL）节点</strong>：除了具有持久（PERSISTENT）节点的特性之外， 子节点的名称还具有顺序性。比如 <code v-pre>/node1/app0000000001</code>、<code v-pre>/node1/app0000000002</code> 。</li>
...
<p>使用 Redis 实现分布式锁的时候，通过过期时间来避免锁无法被释放导致死锁问题的，而 ZooKeeper 直接利用临时节点的特性即可。</p>
<p>假设不使用顺序节点的话，所有尝试获取锁的客户端都会对持有锁的子节点加监听器。当该锁被释放之后，势必会造成所有尝试获取锁的客户端来争夺锁，这样对性能不友好。使用顺序节点之后，只需要监听前一个节点就好了，对性能更友好。</p>
<blockquote>
<p>ZooKeeper通过临时节点解决释放锁逻辑失效问题，通过顺序节点解决顺序唤醒后续节点（避免竞争锁）。</p>
</blockquote>
<h3 id="为什么要设置对前一个节点的监听" tabindex="-1"><a class="header-anchor" href="#为什么要设置对前一个节点的监听" aria-hidden="true">#</a> 为什么要设置对前一个节点的监听？</h3>
<p>当一个节点对应的客户端释放锁之后（也就是前一个节点被删除，监听的是删除事件），通知获取锁失败的客户端（唤醒等待线程，Java中的<code v-pre>wait/notify</code>），让它去获取锁。</p>
</div></template>
