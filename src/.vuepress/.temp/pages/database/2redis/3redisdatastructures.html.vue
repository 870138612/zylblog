...
<h2 id="list-列表" tabindex="-1"><a class="header-anchor" href="#list-列表" aria-hidden="true">#</a> List（列表）</h2>
<p>Redis 中的 List 其实就是链表数据结构的实现，Redis 的 List 的实现为一个 <strong>双向链表</strong>，即可以支持反向查找和遍历，更方便操作，不过带来了部分额外的内存开销。</p>
<p><img src="/markdown/image-20230608171725873.png" alt="image-20230608171725873"></p>
...
<ul>
<li>举例：最新文章、最新动态。</li>
<li>相关命令：<code v-pre>LPUSH</code>、<code v-pre>LRANGE</code>。</li>
</ul>
<p><strong>消息队列</strong></p>
<p>Redis List 数据结构可以用来做消息队列，只是功能过于简单且存在很多缺陷，不建议这样做。</p>
<p>相对来说，Redis 5.0 新增加的一个数据结构 <code v-pre>Stream</code> 更适合做消息队列一些，只是功能依然非常简陋。和专业的消息队列相比，还是有很多欠缺的地方比如消息丢失和堆积问题不好解决。</p>
...
<p>Hash 类似于 JDK1.8 前的 <code v-pre>HashMap</code>，内部实现也差不多(数组 + 链表)。不过，Redis 的 Hash 做了更多优化。</p>
<p><img src="/markdown/image-20230608172142206.png" alt="image-20230608172142206"></p>
...
<blockquote>
<p>Hash数据结构添加需要三个参数，分别是hash的key、键、值，而String类型是两个参数。</p>
</blockquote>
...
<li>相关命令：<code v-pre>HSET</code> （设置单个字段的值）、<code v-pre>HMSET</code>（设置多个字段的值）、<code v-pre>HGET</code>（获取单个字段的值）、<code v-pre>HMGET</code>（获取多个字段的值）。</li>
</ul>
<h2 id="set-集合" tabindex="-1"><a class="header-anchor" href="#set-集合" aria-hidden="true">#</a> Set（集合）</h2>
<p>Redis 中的 Set 类型是一种无序集合，集合中的元素没有先后顺序但都唯一，有点类似于 Java 中的 <code v-pre>HashSet</code> 。基于 Set 能轻易实现交集、并集、差集的操作，比如可以将一个用户所有的关注人存在一个集合中，将其所有粉丝存在一个集合。这样的话，Set 可以非常方便的实现如共同关注、共同粉丝、共同喜好等功能。</p>
<p><img src="/markdown/image-20230608172937070.png" alt="image-20230608172937070"></p>
...
<ul>
<li>举例：网站 UV 统计（数据量巨大的场景还是 <code v-pre>HyperLogLog</code>更适合一些）、文章点赞、动态点赞等场景。</li>
<li>相关命令：<code v-pre>SCARD</code>（获取集合数量） 。</li>
</ul>
<p><strong>需要获取多个数据源交集、并集和差集的场景</strong></p>
<ul>
<li>举例：共同好友(交集)、共同粉丝(交集)、共同关注(交集)、好友推荐（差集）、音乐推荐（差集）、订阅号推荐（差集+交集） 等场景。</li>
<li>相关命令：<code v-pre>SINTER</code>（交集）、<code v-pre>SINTERSTORE</code> （交集）、<code v-pre>SUNION</code> （并集）、<code v-pre>SUNIONSTORE</code>（并集）、<code v-pre>SDIFF</code>（差集）、<code v-pre>SDIFFSTORE</code> （差集）。</li>
</ul>
<p><strong>需要随机获取数据源中的元素的场景</strong></p>
<ul>
<li>举例：抽奖系统、随机点名等场景。</li>
<li>相关命令：<code v-pre>SPOP</code>（随机获取集合中的元素并移除，适合不允许重复中奖的场景）、<code v-pre>SRANDMEMBER</code>（随机获取集合中的元素，适合允许重复中奖的场景）。</li>
</ul>
<h2 id="sorted-set-有序集合" tabindex="-1"><a class="header-anchor" href="#sorted-set-有序集合" aria-hidden="true">#</a> Sorted Set（有序集合）</h2>
<p>Sorted Set 类似于 Set，但和 Set 相比，Sorted Set 增加了一个权重参数 <code v-pre>score</code>，使得集合中的元素能够按 <code v-pre>score</code> 进行有序排列，还可以通过 <code v-pre>score</code> 的范围来获取元素的列表。有点像是 Java 中 <code v-pre>HashMap</code> 和 <code v-pre>TreeSet</code> 的结合体。</p>
<p><img src="/markdown/image-20230608173432109.png" alt="image-20230608173432109"></p>
...
<ul>
<li>举例：各种排行榜比如直播间送礼物的排行榜、朋友圈的微信步数排行榜、游戏中的段位排行榜、话题热度排行榜等等。</li>
<li>相关命令：<code v-pre>ZRANGE</code> (从小到大排序)、 <code v-pre>ZREVRANGE</code> （从大到小排序）、<code v-pre>ZREVRANK</code> (指定元素排名)。</li>
</ul>
...
<h2 id="bitmap" tabindex="-1"><a class="header-anchor" href="#bitmap" aria-hidden="true">#</a> Bitmap</h2>
<p>Bitmap 存储的是连续的二进制数字（0 和 1），通过 Bitmap, 只需要一个 bit 位来表示某个元素对应的值或者状态，key 就是对应元素本身 。8 个 bit 可以组成一个 byte，所以 Bitmap 本身会极大的节省储存空间。</p>
<p><img src="/markdown/image-20230608173918450.png" alt="image-20230608173918450"></p>
...
<li>相关命令：<code v-pre>SETBIT</code>、<code v-pre>GETBIT</code>、<code v-pre>BITCOUNT</code>、<code v-pre>BITOP</code>。</li>
</ul>
...
<p>Redis 提供的 HyperLogLog 占用空间非常非常小，只需要 12k 的空间就能存储接近<code v-pre>2^64</code>个不同元素。并且，Redis 对 HyperLogLog 的存储结构做了优化，采用两种方式计数：</p>
<ul>
<li><strong>稀疏矩阵</strong>：计数较少的时候，占用空间很小。</li>
<li><strong>稠密矩阵</strong>：计数达到某个阈值的时候，占用 12k 的空间。</li>
</ul>
<p>基数计数概率算法为了节省内存并不会直接存储元数据，而是通过一定的概率统计方法预估基数值（集合中包含元素的个数）。因此， HyperLogLog 的计数结果并不是一个精确值，存在一定的误差（标准误差为 <code v-pre>0.81%</code> ）。</p>
...
<li>相关命令：<code v-pre>PFADD</code>、<code v-pre>PFCOUNT</code> 。</li>
</ul>
<h2 id="geospatial-index" tabindex="-1"><a class="header-anchor" href="#geospatial-index" aria-hidden="true">#</a> Geospatial index</h2>
<p>Geospatial index（地理空间索引，简称 GEO） 主要用于存储地理位置信息，基于 Sorted Set 实现。</p>
<p>通过 GEO 可以轻松实现两个位置距离的计算、获取指定位置附近的元素等功能。</p>
...
<h3 id="应用场景-7" tabindex="-1"><a class="header-anchor" href="#应用场景-7" aria-hidden="true">#</a> 应用场景</h3>
<p><strong>需要管理使用地理空间数据的场景</strong></p>
<ul>
<li>举例：查找附近商铺。</li>
<li>相关命令: <code v-pre>GEOADD</code>、<code v-pre>GEORADIUS</code>、<code v-pre>GEORADIUSBYMEMBER</code> 。</li>
</ul>
</div></template>
