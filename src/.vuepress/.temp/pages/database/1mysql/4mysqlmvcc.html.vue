<template><div><h2 id="事务隔离级别" tabindex="-1"><a class="header-anchor" href="#事务隔离级别" aria-hidden="true">#</a> 事务隔离级别</h2>
<p>SQL定义了四个隔离级别：</p>
...
<p>标准的 SQL 隔离级别定义里，REPEATABLE-READ(可重复读)是不可以防止幻读的，但是InnoDB 实现的 <strong>REPEATABLE-READ</strong> 隔离级别其实是可以解决幻读问题发生的，主要有下面两种情况：</p>
<ul>
<li>
<p><strong>快照读</strong>：由 MVCC 机制来保证不出现幻读。</p>
</li>
<li>
<p><strong>当前读</strong>：使用 Next-Key Lock 进行加锁来保证不出现幻读，Next-Key Lock 是行锁（Record Lock）和间隙锁（Gap Lock）的结合，行锁只能锁住已经存在的行，为了避免插入新行，需要依赖间隙锁</p>
</li>
</ul>
<h2 id="一致性非锁定读和锁定读" tabindex="-1"><a class="header-anchor" href="#一致性非锁定读和锁定读" aria-hidden="true">#</a> 一致性非锁定读和锁定读</h2>
<h3 id="一致性非锁定读" tabindex="-1"><a class="header-anchor" href="#一致性非锁定读" aria-hidden="true">#</a> 一致性非锁定读</h3>
<p>对于<strong>一致性非锁定读</strong>的实现，通常做法是加一个版本号或者时间戳字段，在更新数据的同时版本号 + 1 或者更新时间戳。查询时，将当前可见的版本号与对应记录的版本号进行比对，如果记录的版本小于可见版本，则表示该记录可见。</p>
<p>在 <code v-pre>InnoDB</code> 存储引擎中，多版本并发控制就是对非锁定读的实现。如果读取的行正在执行 <code v-pre>DELETE</code> 或 <code v-pre>UPDATE</code> 操作，这时读取操作不会去等待行上锁的释放。相反地，<code v-pre>InnoDB</code> 存储引擎会去读取行的一个快照数据，对于这种读取历史数据的方式，叫它快照读 (snapshot read)</p>
...
<p>如果执行的是下列语句，就是<strong>锁定读</strong></p>
<ul>
...
<ul>
<li><code v-pre>select ... lock in share mode</code>：对记录加 <code v-pre>S</code> 锁，其它事务也可以加<code v-pre>S</code>（共享锁）锁，如果加 <code v-pre>x</code> 锁则会被阻塞。</li>
<li><code v-pre>select ... for update</code>、<code v-pre>insert</code>、<code v-pre>update</code>、<code v-pre>delete</code>：对记录加 <code v-pre>X</code>（排它锁） 锁，且其它事务不能加任何锁。</li>
</ul>
<p>在一致性非锁定读下，即使读取的记录已被其它事务加上 <code v-pre>X</code> 锁，这时记录也是可以被读取的，即读取的快照数据。上面说了，在 <code v-pre>Repeatable Read</code> 下 <code v-pre>MVCC</code> 防止了部分幻读，这边的 “部分” 是指在<strong>一致性非锁定读</strong> 情况下，只能读取到第一次查询之前所插入的数据（根据 Read View 判断数据可见性，Read View 在第一次查询时生成）。但是！如果是<strong>当前读</strong> ，每次读取的都是最新数据，这时如果两次查询中间有其它事务插入数据，就会产生幻读。所以， <strong><code v-pre>InnoDB</code> 在实现<code v-pre>Repeatable Read</code> 时，如果执行的是当前读，则会对读取的记录使用 <code v-pre>Next-key Lock</code> ，来防止其它事务在间隙间插入数据</strong>。</p>
<h2 id="innodb-对-mvcc-的实现" tabindex="-1"><a class="header-anchor" href="#innodb-对-mvcc-的实现" aria-hidden="true">#</a> InnoDB 对 MVCC 的实现</h2>
<p><code v-pre>MVCC</code> 的实现依赖于：<strong>隐藏字段、Read View、undo log</strong>。</p>
<p>在内部实现中，<code v-pre>InnoDB</code> 通过数据行的 <code v-pre>DB_TRX_ID</code> 和 <code v-pre>Read View</code> 来判断数据的可见性，如不可见，则通过数据行的 <code v-pre>DB_ROLL_PTR</code> 找到 <code v-pre>undo log</code> 中的历史版本。每个事务读到的数据版本可能是不一样的，在同一个事务中，用户只能看到该事务创建 <code v-pre>Read View</code> 之前已经提交的修改和该事务本身做的修改</p>
<h3 id="隐藏字段" tabindex="-1"><a class="header-anchor" href="#隐藏字段" aria-hidden="true">#</a> 隐藏字段</h3>
<p>在内部，<code v-pre>InnoDB</code> 存储引擎为每行数据添加了三个隐藏字段：</p>
<ul>
<li>
<p><code v-pre>DB_TRX_ID（6字节）</code>：表示最后一次插入或更新该行的事务 id。此外，<code v-pre>delete</code> 操作在内部被视为更新，只不过会在记录头 <code v-pre>Record header</code> 中的 <code v-pre>deleted_flag</code> 字段将其标记为已删除；</p>
</li>
<li>
<p><code v-pre>DB_ROLL_PTR（7字节）</code> 回滚指针，指向该行的 <code v-pre>undo log</code> 。如果该行未被更新，则为空；</p>
</li>
<li>
<p><code v-pre>DB_ROW_ID（6字节）</code>：如果没有设置主键且该表没有唯一非空索引时，<code v-pre>InnoDB</code> 会使用该 id 来生成聚簇索引。</p>
</li>
</ul>
...
<p><img src="/markdown/image-20230529200011975.png" alt="image-20230529200011975"></p>
...
<ul>
<li>在 RC 隔离级别下的 <strong>每次select</strong> 查询前都生成一个<code v-pre>Read View</code> (m_ids 列表)；</li>
<li>在 RR 隔离级别下只在事务开始后 <strong>第一次select</strong> 数据前生成一个<code v-pre>Read View</code>（m_ids 列表）。</li>
</ul>
<h2 id="mvcc下解决不可重复读问题" tabindex="-1"><a class="header-anchor" href="#mvcc下解决不可重复读问题" aria-hidden="true">#</a> MVCC下解决不可重复读问题</h2>
...
<p>在RR隔离级别下，事务在第一次查询开始的时候会生成一个<code v-pre>Read View</code> 后续复用这个<code v-pre>Read View</code>。</p>
<h2 id="mvcc-next-key-lock防止幻读" tabindex="-1"><a class="header-anchor" href="#mvcc-next-key-lock防止幻读" aria-hidden="true">#</a> MVCC+Next-key Lock防止幻读</h2>
<p><code v-pre>InnoDB</code>存储引擎在RR隔离级别下通过<code v-pre>MVCC</code>和<code v-pre>Next-key Lock</code>来解决幻读问题。</p>
...
<p>☀️详见<a href="https://ylzhong.top/database/1mysql/1mysql.html#mysql-%E9%94%81" target="_blank" rel="noopener noreferrer">MySQL锁<ExternalLinkIcon/></a></p>
<blockquote>
