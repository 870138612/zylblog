<template><div><h2 id="redo-log-重做日志" tabindex="-1"><a class="header-anchor" href="#redo-log-重做日志" aria-hidden="true">#</a> redo log 重做日志</h2>
<p><code v-pre>redo log</code>是重做日志，是<code v-pre>InnoDB</code>引擎独有的，让<code v-pre>MySQL</code>拥有了崩溃恢复的能力。</p>
<p><img src="/markdown/image-20230529154219243.png" alt="image-20230529154219243"></p>
<!-- more -->
<p><code v-pre>MySQL</code>中以页为单位，查询记录的时候会从硬盘中将一页的数据加载，放入<code v-pre>Buffer Pool</code>中。</p>
<p>后续的查找都是从<code v-pre>Buffer Pool</code>中查找。</p>
<p>更新表数据的时候，发现<code v-pre>Buffer Pool</code>里存在要更新的数据，就直接在<code v-pre>Buffer Pool</code>中更新。</p>
<p>然后会把在“<strong>某个数据页上做了什么修改</strong>”记录到重做日志缓存（<code v-pre>redo log buffer</code>）里，刷盘到<code v-pre>redo log</code>文件里。</p>
<p><img src="/markdown/image-20230529162226606.png" alt="image-20230529162226606"></p>
<p>理想情况下，事务一提交就会进行刷盘操作，实际上刷盘的实际是根据策略来进行的。</p>
...
<p><img src="/markdown/image-20230529162523262.png" alt="image-20230529162523262"></p>
<p>也就是说，一个没有提交事务的 <code v-pre>redo log</code> 记录，也可能会刷盘。</p>
<blockquote>
<p>数据修改-&gt;redo log buffer-&gt;redo.file</p>
</blockquote>
...
<p><img src="/markdown/image-20230529162900611.png" alt="image-20230529162900611"></p>
...
<p>如果 <code v-pre>write pos</code> 追上 <code v-pre>checkpoint</code> ，表示<strong>日志文件组</strong>满了，这时候不能再写入新的 <code v-pre>redo log</code> 记录，<code v-pre>MySQL</code> 得停下来，清空一些记录，把 <code v-pre>checkpoint</code> 推进一下。(循环链表)</p>
<p><img src="/markdown/image-20230529163654789.png" alt="image-20230529163654789"></p>
...
<blockquote>
<p>每次变更都需要先修改Buffer Pool，然后master线程（同步、阻塞）以一定频率刷入磁盘。</p>
</blockquote>
<h2 id="binlog-归档日志" tabindex="-1"><a class="header-anchor" href="#binlog-归档日志" aria-hidden="true">#</a> binlog 归档日志</h2>
<p><code v-pre>redo log</code>是物理日志，记录内容是“在某个数据页上做了什么修改”，属于<code v-pre>InnoDB</code>存储引擎。</p>
<p>而<code v-pre>binlog</code>是逻辑日志，记录内容是语句的原始逻辑，属于<code v-pre>MySQL Server</code>层。</p>
<p>不管用什么存储引擎，只要发生了表数据更新，都会产生<code v-pre>binlog</code>日志。</p>
<p>数据库的数据备份、主备、主主、主从都离不开<code v-pre>binlog</code>，需要依靠<code v-pre>binlog</code>来同步数据，保证数据的一致性。</p>
<p><img src="/markdown/image-20230529165800826.png" alt="image-20230529165800826"></p>
...
<p><img src="/markdown/image-20230529170202703.png" alt="image-20230529170202703"></p>
<p>同步数据时，会执行记录的<code v-pre>SQL</code>语句，但是有个问题，<code v-pre>update_time=now()</code>这里会获取当前系统时间，直接执行会导致与原库的数据不一致。</p>
<p>为了解决这种问题，需要指定为<code v-pre>row</code>，记录的内容不再是简单的<code v-pre>SQL</code>语句了，还包含操作的具体数据，记录内容如下。</p>
<p><img src="/markdown/image-20230529170702610.png" alt="image-20230529170702610"></p>
...
<h3 id="写入时机" tabindex="-1"><a class="header-anchor" href="#写入时机" aria-hidden="true">#</a> 写入时机</h3>
...
<p>可以通过<code v-pre>binlog_cache_size</code>参数控制单个线程 binlog cache 大小，如果存储内容超过了这个参数，就要暂存到磁盘（<code v-pre>Swap</code>）。</p>
...
<p>以<code v-pre>update</code>语句为例，假设<code v-pre>id=2</code>的记录，字段<code v-pre>c</code>值是<code v-pre>0</code>，把字段<code v-pre>c</code>值更新成<code v-pre>1</code>，<code v-pre>SQL</code>语句为<code v-pre>update T set c=1 where id=2</code>。</p>
<p>假设执行过程中写完<code v-pre>redo log</code>日志后，<code v-pre>binlog</code>日志写期间发生了异常，会出现什么情况呢？</p>
<p><img src="/markdown/image-20230529173603129.png" alt="image-20230529173603129"></p>
...
<p>从数据库和备份数据库通过<code v-pre>binlog</code>同步数据。</p>
<p><img src="/markdown/image-20230529173719124.png" alt="image-20230529173719124"></p>
...
<p><img src="/markdown/image-20230529173846546.png" alt="image-20230529173846546"></p>
...
<p><img src="/markdown/image-20230529173959416.png" alt="image-20230529173959416"></p>
<p>再看一个场景，<code v-pre>redo log</code>设置<code v-pre>commit</code>阶段发生异常，那会不会回滚事务呢？</p>
<p><img src="/markdown/image-20230529174101634.png" alt="image-20230529174101634"></p>
...
<p>☀️详见<a href="https://https://ylzhong.top/database/1mysql/4mysqlmvcc.html" target="_blank" rel="noopener noreferrer">事务隔离级别和MVCC<ExternalLinkIcon/></a>。</p>
