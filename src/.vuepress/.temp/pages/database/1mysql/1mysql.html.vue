...
<!-- more -->
<h2 id="mysql基础架构" tabindex="-1"><a class="header-anchor" href="#mysql基础架构" aria-hidden="true">#</a> MySQL基础架构</h2>
<p>MySQL主要由下面几个部分构成：</p>
<ul>
<li><strong>连接器</strong>：身份认证和权限相关。</li>
<li><strong>查询缓存</strong>：执行查询语句的时候，会先查询缓存（MySQL 8.0版本后移除）。</li>
<li><strong>分析器</strong>：没有命中缓存的话，SQL语句会经过分析器，分析SQL语句操作，检查语法是否正确。</li>
...
<p>MyISAM只有表级锁，而InnoDB支持行级锁和表级锁，默认为行级锁。MyISAM一锁就是整张表，并发度不及InnoDB。</p>
<ul>
<li>是否支持事务</li>
</ul>
<p>MyISAM不提供事务。</p>
<p>InnoDB提供事务支持，实现了SQL标准定义的四个隔离级别，具有提交和回滚事务的能力，并且InnoDB默认使用REPEAETABLE-READ（可重复读）隔离级别是可以解决幻读问题的（基于MVCC和Next-Key Lock）。</p>
<ul>
<li><strong>是否支持外键</strong></li>
</ul>
<p>MyISAM不支持，而InnoDB支持。外键对于维护数据的一致性非常有帮助，但是在日常开发中不建议使用外键，请通过关系表实现。</p>
<ul>
<li><strong>是否支持数据库异常崩溃后的安全恢复</strong></li>
</ul>
<p>MyISAM不支持，而InnoDB支持。</p>
<p>使用InnoDB的数据库在异常崩溃之后，数据库重新启动的时候会保证数据库恢复到崩溃前的状态，这个恢复过程依赖于<code v-pre>redo log</code>。</p>
<ul>
<li><strong>是否支持MVCC</strong></li>
</ul>
<p>MyISAM不支持，InnoDB支持。</p>
<ul>
<li><strong>索引实现不同</strong></li>
</ul>
<p>MyISAM引擎InnoDB引擎都是使用B+Tree作为索引结构，但是两者实现方式不同。</p>
<p>InnoDB引擎中，其数据文件本身就是索引文件。相比MyISAM，索引文件和数据文件是分离的，其表数据文件本身就是按B+Tree阻止的一个索引结构，树的叶节点data域保存了完整的数据记录。</p>
<ul>
...
<li>缓存建立之后，MySQL的查询缓存系统会跟踪查询中涉及的没一张表，如果这些表发生变化，那么和这张表的所有缓存数据都失效。</li>
</ul>
<p>缓存虽然能提升数据库的查询能力，但是缓存同时也带来了额外的开销，每次查询后都要做一次缓存操作，失效后还要销毁。</p>
<h2 id="mysql-事务" tabindex="-1"><a class="header-anchor" href="#mysql-事务" aria-hidden="true">#</a> MySQL 事务</h2>
<p>事务是逻辑上的一组操作，要么都执行，要么都不执行。</p>
<p>MySQL中开启事务：</p>
<div class="language-mysql line-numbers-mode" data-ext="mysql"><pre v-pre class="language-mysql"><code># 开启一个事务
START TRANSACTION;
# 多条 SQL 语句
SQL1,SQL2...
## 提交事务
...
<li>隔离性<code v-pre>Isolation</code>：并发访问数据库时，一个用户的事物不被其他事务所干扰，各并发事务之间的数据库是独立的。</li>
...
<p><strong>脏读</strong>：读到了没有提交的数据。</p>
<blockquote>
<p><strong>READ-UNCOMMITTED</strong></p>
</blockquote>
<p><strong>不可重复读</strong>：在一个事务内开启多次读，读取条件相同的情况下得到的结果却是不一样的。</p>
<blockquote>
<p><strong>READ-UNCOMMITTED、READ-COMMITTED</strong></p>
</blockquote>
<p><strong>幻读</strong>：读取到了一个原本没有的数据（新插入的数据）。</p>
<blockquote>
...
<ul>
<li><strong>表级锁</strong>：MySQL中锁定粒度最大的锁，对当前操作的整张表加锁，实现简单，资源消耗快，不会出现死锁。并发度低，与存储引擎无关，MyISAM和InnoDB都支持表级锁。</li>
<li><strong>行级锁</strong>：MySQL中锁定粒度最小的一种锁，是针对索引字段加锁，只针对当前操作的行记录进行加锁。行级锁能大大减少数据库操作的冲突。加锁粒度最小，并发度高，但加锁的开销也最大，加锁慢，会出现死锁。行级锁和存储引擎有关，InnoDB支持MyISAM不支持。</li>
</ul>
...
<p><strong>意向锁是表级锁</strong>，共有两种：</p>
<ul>
<li>
...
<p><strong>意向锁是由数据引擎自己维护的，用户无法手动操作意向锁，在为数据行加共享/排他锁之前，InnoDB会先获取该行数据所在数据表对应的意向锁。</strong></p>
<p><strong>意向锁之间是相互兼容的。</strong></p>
...
UPDATE...
DELETE...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="mysql如何存储ip地址" tabindex="-1"><a class="header-anchor" href="#mysql如何存储ip地址" aria-hidden="true">#</a> MySQL如何存储IP地址？</h2>
<p>可以将IP地址转换成整形数据存储，性能更好，占用空间更小。</p>
...
<p>可以使用 <code v-pre>EXPLAIN</code> 命令来分析 SQL 的 <strong>执行计划</strong> 。执行计划是指一条 SQL 语句在经过 MySQL 查询优化器的优化会后，具体的执行方式。</p>
<p><code v-pre>EXPLAIN</code> 并不会真的去执行相关的语句，而是通过 <strong>查询优化器</strong> 对语句进行分析，找出最优的查询方案，并显示对应的信息。</p>
<div class="hint-container info">
<p class="hint-container-title">分析EXPLAIN结果</p>
<ul>
<li>
<p><strong>id</strong>：SELECT标识符，是查询中SELECT的序号，用来标识整个查询中SELECT语句的顺序。id如果相同，从上往下一次执行。</p>
</li>
<li>
<p><strong>select_type</strong>：查询的类型，主要用于区分普通查询、联合查询、子查询等复杂查询。</p>
<ul>
...
<li>
<p><strong>type（重要）</strong>：查询执行的类型，所有值的顺序从最优到最差排序为system &gt; const &gt; eq_ref &gt; ref &gt; fulltext &gt; ref_or_null &gt; index_merge &gt; unique_subquery &gt; index_subquery &gt; range &gt; index &gt; ALL。</p>
<ul>
...
<h2 id="读写分离和分库分表" tabindex="-1"><a class="header-anchor" href="#读写分离和分库分表" aria-hidden="true">#</a> 读写分离和分库分表</h2>
<h3 id="读写分离" tabindex="-1"><a class="header-anchor" href="#读写分离" aria-hidden="true">#</a> 读写分离</h3>
<p>读写分离主要是为了将对数据库的读写操作分散到不同的数据库节点（主节点写，从节点读）上，这样能小幅度提升写性能，大幅度提升读性能。</p>
<h3 id="读写分离会带来什么问题" tabindex="-1"><a class="header-anchor" href="#读写分离会带来什么问题" aria-hidden="true">#</a> 读写分离会带来什么问题？</h3>
<p>读写分离对提升数据库的并发非常有效，但是主库和从库的数据存在延时（<strong>主从同步延迟</strong> ）。</p>
<h3 id="如何实现读写分离" tabindex="-1"><a class="header-anchor" href="#如何实现读写分离" aria-hidden="true">#</a> 如何实现读写分离？</h3>
<ol>
<li>部署多台数据库，选择其中一台作为主数据库，其他的一台或者多台作为从数据库。</li>
<li>保证主数据库和从数据库之间的数据是实时同步的，这个过程称为<strong>主从复制</strong>。</li>
<li>系统将写请求交给主数据库处理，读请求交给从数据库处理。</li>
</ol>
<h3 id="主从复制的原理是什么" tabindex="-1"><a class="header-anchor" href="#主从复制的原理是什么" aria-hidden="true">#</a> 主从复制的原理是什么？</h3>
<ol>
<li>
...
<p><strong>垂直分表</strong>：是对数据表列的拆分，<strong>把一张列比较多的表拆分成多张表</strong>。</p>
<p><strong>水平分表</strong>：是对数据表行的拆分，吧一张行比较多的表拆分为多张表，可以解决单一表数据量过大的问题。</p>
...
<li>数据库的数据占用的空间越来越大，备份时间越来越长。</li>
<li>应用的并发量太大。</li>
</ul>
<h3 id="常见的分片算法有哪些" tabindex="-1"><a class="header-anchor" href="#常见的分片算法有哪些" aria-hidden="true">#</a> 常见的分片算法有哪些？</h3>
<p>分片算法主要解决了数据被水平分片之后，数据应该放在哪个表的问题。</p>
<ul>
<li><strong>哈希分片</strong>：求指定key的哈希，然后根据哈希值确定数据应被放置在哪个表中。哈希分片比较适合随机读写的场景，不太适合经常使用范围查询的场景。</li>
<li><strong>范围分片</strong>：按照特性的范围区间来分配数据。</li>
<li><strong>地理位置分片</strong>：很多NewSQL数据库都支持地理位置分片算法。</li>
<li><strong>融合算法</strong>：灵活组合多种分片算法。</li>
</ul>
