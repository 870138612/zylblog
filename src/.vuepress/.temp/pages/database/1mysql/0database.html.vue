<template><div><h2 id="数据库的三大范式" tabindex="-1"><a class="header-anchor" href="#数据库的三大范式" aria-hidden="true">#</a> 数据库的三大范式</h2>
<h3 id="_1nf" tabindex="-1"><a class="header-anchor" href="#_1nf" aria-hidden="true">#</a> 1NF</h3>
...
<h3 id="_3nf" tabindex="-1"><a class="header-anchor" href="#_3nf" aria-hidden="true">#</a> 3NF</h3>
<p>3NF在2NF的基础上，<strong>消除了非主属性对于码的传递函数依赖</strong>。符合3NF要求的数据库设计，基本上解决了数据冗余过大，插入异常，修改异常，删除异常的问题。比如在关系 R(学号 , 姓名, 系名，系主任)中，学号 → 系名，系名 → 系主任，所以存在非主属性系主任对于学号的传递函数依赖，所以该表的设计，不符合 3NF 的要求。</p>
<h2 id="drop、delete-与-truncate-区别" tabindex="-1"><a class="header-anchor" href="#drop、delete-与-truncate-区别" aria-hidden="true">#</a> drop、delete 与 truncate 区别？</h2>
<ul>
<li><code v-pre>drop</code>(丢弃数据): <code v-pre>drop table 表名</code> ，直接将表都删除掉，在删除表的时候使用。</li>
<li><code v-pre>truncate</code> (清空数据) : <code v-pre>truncate table 表名</code> ，只删除表中的数据，再插入数据的时候自增长 id 又从 1 开始，在清空表中数据的时候使用。</li>
<li><code v-pre>delete</code>（删除数据） : <code v-pre>delete from 表名 where 列名=值</code>，删除某一行的数据，如果不加 <code v-pre>where</code> 子句和<code v-pre>truncate table 表名</code>作用类似。</li>
</ul>
<h2 id="dml-语句和-ddl-语句区别" tabindex="-1"><a class="header-anchor" href="#dml-语句和-ddl-语句区别" aria-hidden="true">#</a> DML 语句和 DDL 语句区别</h2>
<ul>
<li>
<p>DML 是数据库操作语言（Data Manipulation Language）的缩写，是指对数据库中表记录的操作，主要包括表记录的插入、更新、删除和查询，是开发人员日常使用最频繁的操作。</p>
</li>
<li>
