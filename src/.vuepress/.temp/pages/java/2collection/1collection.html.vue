<template><div><h2 id="集合" tabindex="-1"><a class="header-anchor" href="#集合" aria-hidden="true">#</a> 集合</h2>
<p>Java集合由两大接口派生而来，分别是用来存放单一元素的<code v-pre>Collection</code>和用来存放键值对的<code v-pre>Map</code>接口。对于<code v-pre>Collection</code>接口有三个子接口，分别是<code v-pre>List</code>、<code v-pre>Set</code>、<code v-pre>Queue</code>。</p>
<!-- more -->
...
<ul>
<li><code v-pre>List</code>（顺序存储）存储的元素是有序的，可以重复；</li>
<li><code v-pre>Set</code>（独一无二）存储的元素无序，不可重复；</li>
<li><code v-pre>Queue</code>（排队）按特定的排队规则确定元素先后顺序，元素是有序的，可重复的；</li>
<li><code v-pre>Map</code>（用key搜索）使用键值对存储，key是无序的，不可重复的，value是无序的，可重复的。</li>
</ul>
<h3 id="集合框架底层数据结构" tabindex="-1"><a class="header-anchor" href="#集合框架底层数据结构" aria-hidden="true">#</a> 集合框架底层数据结构</h3>
<p><strong>List</strong></p>
<ul>
<li><code v-pre>ArrayList</code>：<code v-pre>Object[]</code>数组；</li>
<li><code v-pre>Vector</code>：<code v-pre>Object[]</code>数组；</li>
<li><code v-pre>LinkedList</code>：双向链表（JDK1.6之前为循环链表，JDK1.7取消）。</li>
</ul>
<p><strong>Set</strong></p>
<ul>
<li><code v-pre>HashSet</code>（无序，唯一）：基于<code v-pre>HashMap</code>实现，底层使用<code v-pre>HashMap</code>保存数据；</li>
<li><code v-pre>LinkedHashMap</code>：<code v-pre>LinkedHashSet</code>是<code v-pre>HashSet</code>子类，内部都是通过<code v-pre>LinkedHashMap</code>实现；</li>
...
<ul>
<li><code v-pre>HashMap</code>：JDK1.8 之前 <code v-pre>HashMap</code> 由数组+链表组成的，数组是 <code v-pre>HashMap</code> 的主体，链表则是主要为了解决哈希冲突而存在。JDK1.8 以后由数组+链表/红黑树组成，当链表长度大于阈值（默认为8），数组长度大于等于64的时候会进行树化，转化为红黑树加快查找。</li>
<li><code v-pre>LinkedHashMap</code>：<code v-pre>LinkedHashMap</code> 继承自 <code v-pre>HashMap</code>，底层是数组+链表/红黑树。<code v-pre>LinkedHashMap</code> 在上面结构的基础上，增加了一条双向链表，使得上面的结构可以保持键值对的插入顺序。同时通过对链表进行相应的操作，实现了访问顺序相关逻辑。</li>
<li><code v-pre>Hashtable</code>：数组+链表组成的，数组是 <code v-pre>Hashtable</code> 的主体，链表则是主要为了解决哈希冲突而存在</li>
</ul>
...
<ul>
<li><code v-pre>ArrayList</code>是<code v-pre>List</code>的主要实现类，底层用<code v-pre>Object[]</code>存储，适用于频繁查找的场景，线程不安全；</li>
<li><code v-pre>Vector</code>是<code v-pre>List</code>的古老实现类，底层用<code v-pre>Object[]</code>存储，线程安全。</li>
</ul>
...
<ul>
<li>首先两者都不是线程安全的实现；</li>
<li><code v-pre>ArrayList</code>底层使用<code v-pre>Object[]</code>存储元素，<code v-pre>LinkedList</code>底层使用双向链表存储数据（JDK1.6之前为循环链表，1.7取消循环）；</li>
<li><code v-pre>ArrayList</code>实现了<code v-pre>RandomAccess</code>接口，可以随机访问，<code v-pre>LinkedList</code>不支持随机访问；</li>
<li>由于<code v-pre>LinkedList</code>内部包含前驱和后继，占用内存比<code v-pre>ArrayList</code>大。</li>
</ul>
...
<li><code v-pre>HashSet</code>底层是哈希表，<code v-pre>LinkedHashSet</code>底层是链表和哈希表，元素的插入满足队列特性，<code v-pre>TreeSet</code>底层是红黑树，元素是有序的。</li>
<li>根据应用场景不同，对元素顺序无要求则用<code v-pre>HashSet</code>，有要求使用<code v-pre>TreeSet</code>，需要满足FIFO使用<code v-pre>LinkedHashSet</code>。</li>
</ul>
<h2 id="queue" tabindex="-1"><a class="header-anchor" href="#queue" aria-hidden="true">#</a> Queue</h2>
<p>队列满足FIFO性质，<code v-pre>Queue</code>因为容量问题导致操作失败后的处理方式不同分为两类方法；</p>
<table>
<thead>
<tr>
<th><code v-pre>Queue</code></th>
<th>抛出异常</th>
<th>返回特殊值</th>
</tr>
</thead>
...
<p><code v-pre>Deque</code>是双端队列，在队列两端都能插入和删除元素，同样根据返回值不同分为两类方法</p>
<table>
<thead>
<tr>
<th><code v-pre>Deque</code></th>
<th>抛出异常</th>
<th>返回特殊值</th>
</tr>
</thead>
...
<h3 id="priorityqueue" tabindex="-1"><a class="header-anchor" href="#priorityqueue" aria-hidden="true">#</a> PriorityQueue</h3>
<p><code v-pre>PriorityQueue</code> 是在 JDK1.5 中被引入的, 其与 <code v-pre>Queue</code> 的区别在于元素出队顺序是与优先级相关的，即总是优先级最高的元素先出队。</p>
<ul>
<li><code v-pre>PriorityQueue</code>使用变长数组存储元素，使用二叉堆实现结构；</li>
<li><code v-pre>PriorityQueue</code>使用堆实现了在O(logn)的时间复杂度内插入和删除元素；</li>
<li><code v-pre>PriorityQueue</code>是线程不安全的，不支持存储<code v-pre>null</code>和<code v-pre>non-comparable</code>对象；</li>
<li><code v-pre>PriorityQueue</code>默认是小顶堆。</li>
</ul>
<p><strong>堆排序的伪代码（重要）</strong>。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//调整函数 建立大顶堆</span>
...
<span class="token keyword">void</span> <span class="token function">headSort</span><span class="token punctuation">(</span><span class="token keyword">int</span> <span class="token class-name">R</span><span class="token punctuation">[</span><span class="token punctuation">]</span><span class="token punctuation">,</span> <span class="token keyword">int</span> n<span class="token punctuation">)</span><span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="blockingqueue" tabindex="-1"><a class="header-anchor" href="#blockingqueue" aria-hidden="true">#</a> BlockingQueue</h3>
