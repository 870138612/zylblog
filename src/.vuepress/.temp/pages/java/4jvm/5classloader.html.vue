<template><div><h2 id="类加载器" tabindex="-1"><a class="header-anchor" href="#类加载器" aria-hidden="true">#</a> 类加载器</h2>
<ul>
<li>类加载器是一个负责加载类的对象，用于实现类加载过程中的第一步。</li>
<li>每一个Java类都有一个引用指向加载它的<code v-pre>ClassLoader</code>。</li>
<li>数组类不是通过<code v-pre>ClassLoader</code>创建的，是由JVM自动产生的</li>
</ul>
<p>类加载器的作用就是加载Java类的字节码（<code v-pre>.class</code>文件）到JVM中，字节码可以使源程序（<code v-pre>.java</code>文件）经过编译而来，也可以是通过工具动态生成或者是从网络上下载而来。</p>
<!-- more -->
<h3 id="类加载器的加载规则" tabindex="-1"><a class="header-anchor" href="#类加载器的加载规则" aria-hidden="true">#</a> 类加载器的加载规则</h3>
...
<h3 id="类加载器总结" tabindex="-1"><a class="header-anchor" href="#类加载器总结" aria-hidden="true">#</a> 类加载器总结</h3>
<p>JVM中内置了三个<code v-pre>ClassLoader</code>：</p>
<ol>
...
<li><strong><code v-pre>AppClassLoader</code>(应用程序类加载器)</strong>：面向用户的加载器，负责加载当前应用 classpath 下的所有 jar 包和类。</li>
</ol>
...
<p><strong><code v-pre>ClassLoader</code> 为<code v-pre>null</code>就是 <code v-pre>BootstrapClassLoader</code> 加载的呢？</strong> 这是因为<code v-pre>BootstrapClassLoader</code> 由 C++ 实现，由于这个 C++ 实现的类加载器在 Java 中是没有与之对应的类的，所以拿到的结果是 null。</p>
<h3 id="自定义类加载器" tabindex="-1"><a class="header-anchor" href="#自定义类加载器" aria-hidden="true">#</a> 自定义类加载器</h3>
<p>除了 <code v-pre>BootstrapClassLoader</code> 其他类加载器均由 Java 实现且全部继承自<code v-pre>java.lang.ClassLoader</code>。如果我们要自定义自己的类加载器，很明显需要继承 <code v-pre>ClassLoader</code>抽象类。</p>
<p><code v-pre>ClassLoader</code> 类有两个关键的方法：</p>
<ul>
<li><code v-pre>protected Class loadClass(String name, boolean resolve)</code>：加载指定二进制名称的类，<strong>实现了双亲委派机制</strong> 。<code v-pre>name</code> 为类的二进制名称，<code v-pre>resove</code> 如果为 true，在加载时调用 <code v-pre>resolveClass(Class&lt;?&gt; c)</code> 方法解析该类。</li>
<li><code v-pre>protected Class findClass(String name)</code>：根据类的二进制名称来查找类，默认实现是空方法。</li>
</ul>
<p>如果不想打破双亲委派模型，就重写 <code v-pre>ClassLoader</code> 类中的 <code v-pre>findClass()</code> 方法即可，无法被父类加载器加载的类最终会通过这个方法被加载。但是，如果想打破双亲委派模型则需要重写 <code v-pre>loadClass()</code> 方法。</p>
<h2 id="双亲委派" tabindex="-1"><a class="header-anchor" href="#双亲委派" aria-hidden="true">#</a> 双亲委派</h2>
<p><code v-pre>ClassLoader</code> 类使用委托模型来搜索类和资源。每个 <code v-pre>ClassLoader</code> 实例都有一个相关的父类加载器。需要查找类或资源时，<code v-pre>ClassLoader</code> 实例会在试图亲自查找类或资源之前，<strong>将搜索类或资源的任务委托给其父类加载器</strong>。 虚拟机中被称为 &quot;bootstrap class loader&quot;的内置类加载器本身没有父类加载器，但是可以作为 <code v-pre>ClassLoader</code> 实例的父类加载器。</p>
<ul>
<li>
<p><code v-pre>ClassLoader</code> 类使用委托模型来搜索类和资源。</p>
</li>
<li>
<p>双亲委派模型要求除了顶层的启动类加载器外，<strong>其余的类加载器都应有自己的父类加载器</strong>。</p>
</li>
<li>
<p><code v-pre>ClassLoader</code> 实例会在试图亲自查找类或资源之前，将搜索类或资源的任务委托给其父类加载器。</p>
</li>
</ul>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">abstract</span> <span class="token keyword">class</span> <span class="token class-name">ClassLoader</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">.</span><span class="token punctuation">.</span><span class="token punctuation">.</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="双亲委派模型的执行流程" tabindex="-1"><a class="header-anchor" href="#双亲委派模型的执行流程" aria-hidden="true">#</a> 双亲委派模型的执行流程</h3>
...
<span class="token punctuation">{</span>
        ...
        <span class="token keyword">if</span> <span class="token punctuation">(</span>c <span class="token operator">==</span> <span class="token keyword">null</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
            ...
            <span class="token keyword">try</span> <span class="token punctuation">{</span>
                <span class="token keyword">if</span> <span class="token punctuation">(</span>parent <span class="token operator">!=</span> <span class="token keyword">null</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
                    ...
                    <span class="token comment">//重写loadClass方法	就能破坏双亲委派</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>每当一个类加载器接收到加载请求的时候，会先将请求转发给父类加载器，在父类加载器没有找到所请求的类情况下，该类才会尝试去加载。</p>
<p>执行流程：</p>
<ul>
<li>在类加载的时候，先判断这个类是否已经加载过了，加载过了会直接返回，否则才会尝试加载。</li>
<li>类加载器在进行类加载的时候，它首先不会尝试去加载这个类，而是把请求委派给父类加载器去完成。最终的请求都会到顶层的启动类加载器<code v-pre>BootstrapClassLoader</code>中。</li>
<li>只有父类加载器反馈不能完成这个加载请求的时候，子加载器才会尝试自己加载。</li>
</ul>
<p>☀️ JVM检查两个Java类是否相同的依据：检查类的全名是否相同，还要看类的加载器是否相同，只有两者都是一样的才认为是相同的Java类。即使两个类来源于同一个<code v-pre>Class</code>文件，被同一个虚拟机加载，只要加载它们的类加载器不同，则这两个类不同。</p>
<h3 id="双亲委派的好处" tabindex="-1"><a class="header-anchor" href="#双亲委派的好处" aria-hidden="true">#</a> 双亲委派的好处</h3>
<p>双亲委派模式，保证了Java程序的稳定运行，可以避免类的重复加载，也保证了Java的核心API不会被修改。</p>
<p>例如自己写一个<code v-pre>java.lang.Object</code>类，在程序运行的时候就会产生两个<code v-pre>Object</code>类，一个是JRE里面的，一个是自己写的，使用双亲委派通过启动类加载器<code v-pre>BootStrapClassLoader</code>发现这个类已经加载了，则会直接返回，不会加载自己写的<code v-pre>Object</code>。</p>
<h3 id="打破双亲委派模型方法" tabindex="-1"><a class="header-anchor" href="#打破双亲委派模型方法" aria-hidden="true">#</a> 打破双亲委派模型方法</h3>
<p>自定义加载器的话，需要继承 <code v-pre>ClassLoader</code> 。如果不想打破双亲委派模型，就重写 <code v-pre>ClassLoader</code> 类中的 <code v-pre>findClass()</code> 方法即可，无法被父类加载器加载的类最终会通过这个方法被加载。但是，如果想打破双亲委派模型则需要重写 <code v-pre>loadClass()</code> 方法。</p>
<blockquote>
<p>类加载器在进行类加载的时候，它首先不会自己去尝试加载这个类，而是把请求委派给父类加载器去完成（调用父类的<code v-pre>loadClass()</code>方法来加载类）。</p>
</blockquote>
</div></template>
