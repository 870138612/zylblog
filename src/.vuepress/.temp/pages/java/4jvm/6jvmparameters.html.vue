<template><div><h2 id="堆内存相关" tabindex="-1"><a class="header-anchor" href="#堆内存相关" aria-hidden="true">#</a> 堆内存相关</h2>
<h3 id="指定堆内存-xms和-xmx" tabindex="-1"><a class="header-anchor" href="#指定堆内存-xms和-xmx" aria-hidden="true">#</a> 指定堆内存<code v-pre>-Xms</code>和<code v-pre>-Xmx</code></h3>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>-Xms2G -Xmx5G
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><!-- more -->
<h3 id="指定新生代内存-young-generation" tabindex="-1"><a class="header-anchor" href="#指定新生代内存-young-generation" aria-hidden="true">#</a> 指定新生代内存（Young Generation）</h3>
<p>一共有两种指定新生代内存大小的办法：</p>
<p><strong>1. 通过<code v-pre>-XX:NewSize</code>和<code v-pre>-XX:MaxNewSize</code>指定</strong></p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>-XX:NewSize=256m
-XX:MaxNewSize=1024m
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>JDK 1.8 的时候，方法区（HotSpot 的永久代）被彻底移除了（JDK1.7 就已经开始了），取而代之是元空间，<strong>元空间使用的是本地内存</strong>。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>Metaspace 由于使用不断扩容到<code v-pre>-XX:MetaspaceSize</code>参数指定的量，就会发生 Full GC，且之后每次 Metaspace 扩容都会发生 Full GC。</p>
<h2 id="垃圾收集器相关" tabindex="-1"><a class="header-anchor" href="#垃圾收集器相关" aria-hidden="true">#</a> 垃圾收集器相关</h2>
...
<p>可以使用以下参数声明这些实现：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token operator">-</span><span class="token constant">XX</span><span class="token operator">:</span><span class="token operator">+</span><span class="token class-name">UseSerialGC</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="gc-日志记录" tabindex="-1"><a class="header-anchor" href="#gc-日志记录" aria-hidden="true">#</a> GC 日志记录</h3>
<p>生产环境上，或者其他要测试 GC 问题的环境上，一定会配置上打印 GC 日志的参数，便于分析 GC 相关的问题。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code># 必选
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="处理-oom" tabindex="-1"><a class="header-anchor" href="#处理-oom" aria-hidden="true">#</a> 处理 OOM</h2>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token operator">-</span><span class="token constant">XX</span><span class="token operator">:</span><span class="token operator">+</span><span class="token class-name">HeapDumpOnOutOfMemoryError</span>
...
<li>
<p><strong>HeapDumpPath</strong> 表示要写入文件的路径; 可以给出任何文件名; 但是，如果 JVM 在名称中找到一个 <code v-pre>&lt;pid&gt;</code> 标记，则当前进程的进程 id 将附加到文件名中，并使用<code v-pre>.hprof</code>格式</p>
</li>
<li>
<p><strong>OnOutOfMemoryError</strong> 用于发出紧急命令，以便在内存不足的情况下执行; 应该在 <code v-pre>cmd args</code> 空间中使用适当的命令。例如，如果我们想在内存不足时重启服务器，我们可以设置参数: <code v-pre>-XX:OnOutOfMemoryError=&quot;shutdown -r&quot;</code> 。</p>
</li>
<li>
<p><strong>UseGCOverheadLimit</strong> 是一种策略，它限制在抛出 OutOfMemory 错误之前在 GC 中花费的 VM 时间的比例</p>
</li>
</ul>
<h2 id="其他" tabindex="-1"><a class="header-anchor" href="#其他" aria-hidden="true">#</a> 其他</h2>
<p><code v-pre>-server</code> : 启用“ Server Hotspot VM”; 此参数默认用于 64 位 JVM</p>
<p><code v-pre>-XX:+UseStringDeduplication</code> : <em>Java 8u20</em> 引入了这个 JVM 参数，通过创建太多相同 String 的实例来减少不必要的内存使用; 这通过将重复 String 值减少为单个全局 <code v-pre>char []</code> 数组来优化堆内存。</p>
<p><code v-pre>-XX:+UseLWPSynchronization</code>: 设置基于 LWP (轻量级进程)的同步策略，而不是基于线程的同步。</p>
<p>``-XX:LargePageSizeInBytes `: 设置用于 Java 堆的较大页面大小; 它采用 GB/MB/KB 的参数; 页面大小越大，我们可以更好地利用虚拟内存硬件资源; 然而，这可能会导致 PermGen 的空间大小更大，这反过来又会迫使 Java 堆空间的大小减小。</p>
<p><code v-pre>-XX:MaxHeapFreeRatio</code> : 设置 GC 后, 堆空闲的最大百分比，以避免收缩。</p>
<p><code v-pre>-XX:SurvivorRatio</code> : eden/survivor 空间的比例, 例如<code v-pre>-XX:SurvivorRatio=6</code> 设置每个 survivor 和 eden 之间的比例为 1:6。</p>
<p><code v-pre>-XX:+UseLargePages</code> : 如果系统支持，则使用大页面内存; 请注意，如果使用这个 JVM 参数，OpenJDK 7 可能会崩溃。</p>
<p><code v-pre>-XX:+UseStringCache</code> : 启用 String 池中可用的常用分配字符串的缓存。</p>
<p><code v-pre>-XX:+UseCompressedStrings</code> : 对 String 对象使用 <code v-pre>byte []</code> 类型，该类型可以用纯 ASCII 格式表示。</p>
<p><code v-pre>-XX:+OptimizeStringConcat</code> : 它尽可能优化字符串串联操作。</p>
</div></template>
