<template><div><p>AQS 的全称为 <code v-pre>AbstractQueuedSynchronizer</code> ，翻译过来的意思就是抽象队列同步器。这个类在 <code v-pre>java.util.concurrent.locks</code> 包下面。</p>
<!-- more -->
<p>AQS 就是一个抽象类，主要用来构建锁和同步器。</p>
<p>比如 <code v-pre>ReentrantLock</code>，<code v-pre>Semaphore</code>，其他的诸如 <code v-pre>ReentrantReadWriteLock</code>，<code v-pre>SynchronousQueue</code>等等皆是基于 AQS 的。</p>
<h2 id="aqs原理" tabindex="-1"><a class="header-anchor" href="#aqs原理" aria-hidden="true">#</a> AQS原理</h2>
<h3 id="aqs核心思想" tabindex="-1"><a class="header-anchor" href="#aqs核心思想" aria-hidden="true">#</a> AQS核心思想</h3>
...
<p>AQS 定义两种资源共享方式：<code v-pre>Exclusive</code>（独占，只有一个线程能执行，如<code v-pre>ReentrantLock</code>）和<code v-pre>Share</code>（共享，多个线程可同时执行，如<code v-pre>Semaphore</code>/<code v-pre>CountDownLatch</code>）。</p>
<h2 id="semaphore-信号量" tabindex="-1"><a class="header-anchor" href="#semaphore-信号量" aria-hidden="true">#</a> Semaphore（信号量）</h2>
<p><code v-pre>synchronized</code> 和 <code v-pre>ReentrantLock</code> 都是一次只允许一个线程访问某个资源，而<code v-pre>Semaphore</code>(信号量)可以用来控制同时访问特定资源的线程数量。</p>
<p>Semaphore 的使用简单，假设有 N(N&gt;5) 个线程来获取 <code v-pre>Semaphore</code> 中的共享资源，下面的代码表示同一时刻 N 个线程中只有 5 个线程能获取到共享资源，其他线程都会阻塞，只有获取到共享资源的线程才能执行。等到有线程释放了共享资源，其他阻塞的线程才能获取到。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>Semaphore</code> 有两种模式：</p>
<ul>
<li><strong>公平模式：</strong> 调用 <code v-pre>acquire()</code> 方法的顺序就是获取许可证的顺序，遵循 FIFO；</li>
<li><strong>非公平模式：</strong> 抢占式的，默认模式。</li>
</ul>
<p><strong>原理：</strong></p>
<p><code v-pre>Semaphore</code> 是共享锁的一种实现，它默认构造 AQS 的 <code v-pre>state</code> 值为 <code v-pre>permits</code>，你可以将 <code v-pre>permits</code> 的值理解为许可证的数量，只有拿到许可证的线程才能执行。</p>
<ul>
<li>
<p><strong>P操作</strong>：调用<code v-pre>semaphore.acquire()</code> ，线程尝试获取许可证，如果 <code v-pre>state &gt;= 0</code> 的话，则表示可以获取成功。如果获取成功的话，使用 CAS 操作去修改 <code v-pre>state</code> 的值 <code v-pre>state=state-1</code>。如果 <code v-pre>state&lt;0</code> 的话，则表示许可证数量不足。此时会创建一个 Node 节点加入阻塞队列，挂起当前线程。</p>
</li>
<li>
<p><strong>V操作</strong>：调用<code v-pre>semaphore.release()</code> ，线程尝试释放许可证，并使用 CAS 操作去修改 <code v-pre>state</code> 的值 <code v-pre>state=state+1</code>。释放许可证成功之后，同时会唤醒同步队列中的一个线程。被唤醒的线程会重新尝试去修改 <code v-pre>state</code> 的值 <code v-pre>state=state-1</code> ，如果 <code v-pre>state&gt;=0</code> 则获取令牌成功，否则重新进入阻塞队列，挂起线程。</p>
</li>
</ul>
<p>除了 <code v-pre>acquire()</code> 方法之外，另一个比较常用的与之对应的方法是 <code v-pre>tryAcquire()</code> 方法，该方法如果获取不到许可就立即返回 false。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><strong>两种典型用法：</strong></p>
<ol>
<li>等待n个线程完成任务之后，主线程继续工作。</li>
</ol>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>子线程<span class="token number">1</span>：
...
<li>等待n个线程准备，在执行任务前先<code v-pre>countDownLatch.awit()</code>，然后主线程调用<code v-pre>countDown()</code>计数值变0，n个线程同时被唤醒。</li>
</ol>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="cyclicbarrier-循环栅栏" tabindex="-1"><a class="header-anchor" href="#cyclicbarrier-循环栅栏" aria-hidden="true">#</a> CyclicBarrier(循环栅栏)</h2>
<p><code v-pre>CyclicBarrier</code> 内部通过一个 <code v-pre>count</code> 变量作为计数器，<code v-pre>count</code> 的初始值为 <code v-pre>parties</code> 属性的初始化值，每当一个线程到了栅栏这里了，那么就将计数器减 1，并将自身阻塞。如果 count 值为 0 了，表示这是这一代最后一个线程到达栅栏，此时才会将栅栏放开，让所有线程继续执行下去。</p>
<p>使用<code v-pre>await()</code>方法告诉<code v-pre>CyclicBarrier</code> 已经到达了屏障，然后当前线程被阻塞。当<code v-pre>count</code>值为0则会将被阻塞的线程唤醒。</p>
</div></template>
