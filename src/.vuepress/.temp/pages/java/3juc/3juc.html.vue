...
<h3 id="threadlocal-有什么用" tabindex="-1"><a class="header-anchor" href="#threadlocal-有什么用" aria-hidden="true">#</a> ThreadLocal 有什么用？</h3>
<p><code v-pre>ThreadLocal</code>类主要解决的就是让每个线程绑定自己的值，可以将<code v-pre>ThreadLocal</code>类形象的比喻成存放数据的盒子，盒子中可以存储每个线程的私有数据。</p>
<p>如果创建了一个<code v-pre>ThreadLocal</code>变量，那么访问这个变量的每一个线程都有这个变量的副本。可以使用 <code v-pre>get()</code> 和 <code v-pre>set()</code> 方法来获取默认值或将其值更改为当前线程所存的副本的值，从而避免了线程安全问题。</p>
<!-- more -->
<h3 id="threadlocal原理" tabindex="-1"><a class="header-anchor" href="#threadlocal原理" aria-hidden="true">#</a> ThreadLocal原理</h3>
<p><code v-pre>Thread</code>类源码：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>变量是放在了当前线程的<code v-pre>ThreadLocalMap</code>中，并不是存在<code v-pre>ThreadLocal</code>中,<code v-pre>ThreadLocal</code>可以理解为<code v-pre>ThreadLocalMap</code>的封装。</p>
...
<p><img src="https://blog-1312634242.cos.ap-shanghai.myqcloud.com/markdown/17195626.jpg" alt="17195626"></p>
...
<h3 id="什么是线程池" tabindex="-1"><a class="header-anchor" href="#什么是线程池" aria-hidden="true">#</a> 什么是线程池?</h3>
<p>线程池是管理一系列线程的资源池。当有任务要处理时，直接从线程池中获取线程来处理，处理完之后线程并不会立即被销毁，而是等待下一个任务。</p>
<h3 id="为什么要用线程池" tabindex="-1"><a class="header-anchor" href="#为什么要用线程池" aria-hidden="true">#</a> 为什么要用线程池？</h3>
<p>池化技术是为了减少每次获取资源的消耗，提高资源利用率。</p>
<p>使用线程池的好处：</p>
<ul>
<li>降低资源消耗；</li>
<li>提高响应速度；</li>
...
<h3 id="如何创建线程池" tabindex="-1"><a class="header-anchor" href="#如何创建线程池" aria-hidden="true">#</a> 如何创建线程池？</h3>
<ul>
<li>通过<code v-pre>ThreadPoolExecutor</code>构造函数来创建；</li>
<li>通过 <code v-pre>Executor</code> 框架的工具类 <code v-pre>Executors</code> 来创建。</li>
</ul>
<p>不同类型的<code v-pre>ThreadPoolExecutor</code>:</p>
<ul>
<li>
<p><strong><code v-pre>FixedThreadPool</code></strong>：该方法返回一个固定线程数量的线程池。该线程池中的线程数量始终不变。当有一个新的任务提交时，线程池中若有空闲线程，则立即执行。若没有，则新的任务会被暂存在一个任务队列中，待有线程空闲时，便处理在任务队列中的任务。<strong>核心线程数为n，最大线程数为n</strong>。</p>
</li>
<li>
<p><strong><code v-pre>SingleThreadExecutor</code>：</strong> 该方法返回一个只有一个线程的线程池。若多余一个任务被提交到该线程池，任务会被保存在一个任务队列中，待线程空闲，按先入先出的顺序执行队列中的任务。<strong>核心线程数为1，最大线程数为1。</strong></p>
</li>
<li>
<p><strong><code v-pre>CachedThreadPool</code>：</strong> 该方法返回一个可根据实际情况调整线程数量的线程池。线程池的线程数量不确定，但若有空闲线程可以复用，则会优先使用可复用的线程。若所有线程均在工作，又有新的任务提交，则会创建新的线程处理任务。所有线程在当前任务执行完毕后，将返回线程池进行复用。<strong>核心线程数为0，最大线程数为<code v-pre>Integer.MAX_VALUE</code></strong>。</p>
</li>
<li>
<p><strong><code v-pre>ScheduledThreadPool</code></strong>：该返回一个用来在给定的延迟后运行任务或者定期执行任务的线程池。<strong>核心线程数为n，最大线程数为<code v-pre>Integer.MAX_VALUE</code></strong>。</p>
</li>
</ul>
<table>
<thead>
...
<th style="text-align:center">任务队列</th>
<th style="text-align:center">可创建的最大线程数</th>
</tr>
</thead>
<tbody>
<tr>
<td><code v-pre>FixedThreadPool</code></td>
<td style="text-align:center">n</td>
<td style="text-align:center">n</td>
<td style="text-align:center"><code v-pre>LinkedBlockingQueue</code>无界队列</td>
<td style="text-align:center">n</td>
</tr>
<tr>
<td><code v-pre>SingleThreadExecutor</code></td>
...
<tr>
<td><code v-pre>CachedThreadPool</code></td>
<td style="text-align:center">0</td>
<td style="text-align:center">Integer.MAX_VALUE</td>
<td style="text-align:center"><code v-pre>SynchronousQueue</code>同步队列</td>
<td style="text-align:center">Integer.MAX_VALUE</td>
</tr>
<tr>
<td><code v-pre>ScheduledThreadPool</code></td>
<td style="text-align:center">n</td>
<td style="text-align:center">Integer.MAX_VALUE</td>
<td style="text-align:center"><code v-pre>DelayedWorkQueue</code>延迟阻塞队列</td>
<td style="text-align:center">n</td>
</tr>
</tbody>
</table>
...
<p><strong>最大线程数</strong>是线程池参数，与<strong>可创建的最大线程数</strong>不是一个意思。</p>
</div>
<h3 id="线程池的主要参数" tabindex="-1"><a class="header-anchor" href="#线程池的主要参数" aria-hidden="true">#</a> 线程池的主要参数</h3>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">int</span> corePoolSize<span class="token punctuation">,</span><span class="token comment">//线程池的核心线程数量</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong><code v-pre>ThreadPoolExecutor</code> 3 个最重要的参数：</strong></p>
<ul>
<li><strong><code v-pre>corePoolSize</code> :</strong> 任务队列未达到队列容量时，最大可以同时运行的线程数量。</li>
<li><strong><code v-pre>maximumPoolSize</code> :</strong> 任务队列中存放的任务达到队列容量的时候，当前可以同时运行的线程数量变为最大线程数。</li>
<li><strong><code v-pre>workQueue</code>:</strong> 新任务来的时候会先判断当前运行的线程数量是否达到核心线程数，如果达到的话，新任务就会被存放在队列中。</li>
</ul>
<p><code v-pre>ThreadPoolExecutor</code>其他常见参数 :</p>
<ul>
<li><strong><code v-pre>keepAliveTime</code></strong>:线程池中的线程数量大于 <code v-pre>corePoolSize</code> 的时候，如果这时没有新的任务提交，核心线程外的线程不会立即销毁，而是会等待，直到等待的时间超过了 <code v-pre>keepAliveTime</code>才会被回收销毁；</li>
...
<li><strong><code v-pre>handler</code></strong> :饱和策略。</li>
</ul>
<h3 id="线程池的饱和策略有哪些" tabindex="-1"><a class="header-anchor" href="#线程池的饱和策略有哪些" aria-hidden="true">#</a> 线程池的饱和策略有哪些？</h3>
<p>如果当前同时运行的线程数量达到最大线程数量并且队列也已经被放满了任务时，新来的任务将会被：</p>
<ul>
<li>
<p><strong><code v-pre>ThreadPoolExecutor.AbortPolicy</code>：</strong> 抛出 <code v-pre>RejectedExecutionException</code>来拒绝新任务的处理。</p>
</li>
<li>
<p><strong><code v-pre>ThreadPoolExecutor.CallerRunsPolicy</code>：</strong> 调用执行自己的线程运行任务，也就是直接在调用<code v-pre>execute</code>方法的线程中运行(<code v-pre>run</code>)被拒绝的任务，如果执行程序已关闭，则会丢弃该任务。因此这种策略会降低对于新任务提交速度，影响程序的整体性能。如果您的应用程序可以承受此延迟并且你要求任何一个任务请求都要被执行的话，你可以选择这个策略。</p>
</li>
<li>
<p><strong><code v-pre>ThreadPoolExecutor.DiscardPolicy</code>：</strong> 不处理新任务，直接丢弃掉。</p>
</li>
<li>
<p><strong><code v-pre>ThreadPoolExecutor.DiscardOldestPolicy</code>：</strong> 此策略将丢弃最早的未处理的任务请求。</p>
</li>
</ul>
<h3 id="线程池中常用的阻塞队列有哪些" tabindex="-1"><a class="header-anchor" href="#线程池中常用的阻塞队列有哪些" aria-hidden="true">#</a> 线程池中常用的阻塞队列有哪些？</h3>
...
<li>
<p>如果当前运行的线程数已经等同于最大线程数了，新建线程将会使当前运行的线程超出最大线程数，那么当前任务会被拒绝，饱和策略会调用<code v-pre>RejectedExecutionHandler.rejectedExecution()</code>方法。</p>
</li>
</ul>
<h3 id="如何设定线程池的大小" tabindex="-1"><a class="header-anchor" href="#如何设定线程池的大小" aria-hidden="true">#</a> 如何设定线程池的大小？</h3>
<p><strong>CPU 密集型任务(N+1)</strong>：N表示CPU核心数，比N多1是为了处理线程偶然发生的缺页中断，或者是其他原因导致任务暂停带来的影响。</p>
<p><strong>I/O 密集型任务(2N)</strong>：系统大部分时间用来IO交互，而在处理IO的时候不会占用CPU资源，这时可将CPU资源交给其他任务，因此创建N+N个核心线程。</p>
<h2 id="future" tabindex="-1"><a class="header-anchor" href="#future" aria-hidden="true">#</a> Future</h2>
<h3 id="future类有什么用" tabindex="-1"><a class="header-anchor" href="#future类有什么用" aria-hidden="true">#</a> Future类有什么用？</h3>
<p><code v-pre>Future</code> 类是异步思想的典型运用，主要用在一些需要执行耗时任务的场景，避免程序一直原地等待耗时任务执行完成，执行效率太低。</p>
<p><strong>简单理解</strong>：有一个任务，提交给了 <code v-pre>Future</code> 来处理。任务执行期间自己可以去做任何想做的事情。并且，在这期间还可以取消任务以及获取任务的执行状态。一段时间之后，可以从 <code v-pre>Future</code> 那里直接取出任务执行结果。</p>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token class-name">FutureTask</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">Object</span><span class="token punctuation">></span></span> callableTask <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">FutureTask</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token punctuation">></span></span><span class="token punctuation">(</span><span class="token keyword">new</span> <span class="token class-name">Callable</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">Object</span><span class="token punctuation">></span></span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token keyword">public</span> <span class="token class-name">Object</span> <span class="token function">call</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token keyword">throws</span> <span class="token class-name">Exception</span> <span class="token punctuation">{</span>
        <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token string">"Callable对象"</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token class-name">FutureTask</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">String</span><span class="token punctuation">></span></span> task <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">FutureTask</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token punctuation">></span></span><span class="token punctuation">(</span><span class="token keyword">new</span> <span class="token class-name">Runnable</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
     ...
     <span class="token keyword">public</span> <span class="token keyword">void</span> <span class="token function">run</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
         <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token string">"Runnable对象"</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
     <span class="token punctuation">}</span>
<span class="token punctuation">}</span><span class="token punctuation">,</span> <span class="token string">"result"</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<p><code v-pre>FutureTask</code>相当于对<code v-pre>Callable</code> 进行了封装，管理着任务执行的情况，存储了 <code v-pre>Callable</code> 的 <code v-pre>call</code> 方法的任务执行结果。</p>
<h3 id="completablefuture-类有什么用" tabindex="-1"><a class="header-anchor" href="#completablefuture-类有什么用" aria-hidden="true">#</a> CompletableFuture 类有什么用？</h3>
<p><code v-pre>Future</code> 在实际使用过程中存在一些局限性比如不支持异步任务的编排组合、获取计算结果的 <code v-pre>get()</code> 方法为阻塞调用。</p>
<p>Java 8 才被引入<code v-pre>CompletableFuture</code> 类可以解决<code v-pre>Future</code> 的这些缺陷。<code v-pre>CompletableFuture</code> 除了提供了更为好用和强大的 <code v-pre>Future</code> 特性之外，还提供了函数式编程、<strong>异步任务编排组合</strong>（可以将多个异步任务串联起来，组成一个完整的链式调用）等能力。</p>
...
<span class="token class-name">CompletableFuture</span><span class="token punctuation">.</span><span class="token function">allOf</span><span class="token punctuation">(</span>future1<span class="token punctuation">,</span>future2<span class="token punctuation">,</span>future3<span class="token punctuation">,</span>future4<span class="token punctuation">,</span>future5<span class="token punctuation">)</span><span class="token punctuation">.</span><span class="token function">get</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
