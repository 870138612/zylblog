<template><div><h2 id="java内存模型" tabindex="-1"><a class="header-anchor" href="#java内存模型" aria-hidden="true">#</a> Java内存模型</h2>
<h3 id="指令重排序" tabindex="-1"><a class="header-anchor" href="#指令重排序" aria-hidden="true">#</a> 指令重排序</h3>
<ul>
...
<!-- more -->
<h3 id="什么是java内存模型-为什么需要java内存模型" tabindex="-1"><a class="header-anchor" href="#什么是java内存模型-为什么需要java内存模型" aria-hidden="true">#</a> 什么是Java内存模型？为什么需要Java内存模型？</h3>
<p>JMM是Java定义的并发编程相关的一组规范，除了抽象了线程和内存之间的关系之外，还规定了Java源代码到CPU可执行指令这个转化过程要遵循哪些和并发相关的原则和规范，主要目的是为了简化编程，增强代码的可移植性。</p>
...
<ul>
<li>Java内存结构和运行时区域有关，定义了JVM在运行时如何分区存储数据，例如堆主要用来存放对象实例。</li>
<li>JMM和Java的并发编程有关，抽象了线程和内存之间的关系，规定了Java源代码到CPU可执行指令这个转化过程要遵循哪些并发相关的原则和规范，目的主要是简化多线程编程，增强程序的可移植性</li>
</ul>
<h3 id="happens-before" tabindex="-1"><a class="header-anchor" href="#happens-before" aria-hidden="true">#</a> happens-before</h3>
<p>前一个操作的结果应该对后一个操作是可见的，无论这两个操作是不是在同一个线程里。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在线程并发执行的时候，可能1 2 操作的结果还没有存入变量中，3操作已经开始执行，这时得到的结果就不是正确结果。（指令流水线数据相关）</p>
<h3 id="happens-before-常见规则" tabindex="-1"><a class="header-anchor" href="#happens-before-常见规则" aria-hidden="true">#</a> happens-before 常见规则</h3>
<ol>
<li>程序顺序规则：一个线程内，书写在前的操作happens-before于书写在后面的操作；</li>
<li>解锁规则：解锁happens-before加锁；</li>
<li>volatile变量规则：对于一个volatile变量的写操作happens-before后面对这个变量的读操作，也就是对这个变量的修改对其后的所有操作都可见；</li>
<li>传递规则：如果A happens-before B，B happens-before C，则A happens-before C；</li>
<li>线程启动规则：Thread对象的<code v-pre>start()</code>方法happens-before这个线程的每一个操作。</li>
</ol>
<p>如果两个操作不满足上述条件的任意一个，则这两个操作就没有顺序保障，JVM可以对这两个操作进行重排序。</p>
<h2 id="并发编程的三个特性" tabindex="-1"><a class="header-anchor" href="#并发编程的三个特性" aria-hidden="true">#</a> 并发编程的三个特性</h2>
<p><strong>原子性：</strong></p>
<p>一次操作或者多次操作，要么所有的操作都能全部执行不会收到任何外界因素干扰而中断，要么都不执行。</p>
<p><code v-pre>synchronized</code>和各种<code v-pre>Lock</code>来实现原子性。</p>
<p><strong>可见性：</strong></p>
<p>当一个线程对共享变量进行修改，那么另外的线程都是能立即看到的。</p>
<p><code v-pre>synchronized</code>、<code v-pre>volatile</code>、各种<code v-pre>Lock</code>实现可见性。</p>
<p><strong>有序性：</strong></p>
<p>由于指令重排序问题，代码的执行顺序未必就是编写代码的顺序。</p>
<p>使用<code v-pre>volatile</code>关键字可以禁止指令进行重排序优化。</p>
<h2 id="volatile关键字" tabindex="-1"><a class="header-anchor" href="#volatile关键字" aria-hidden="true">#</a> volatile关键字</h2>
<p>在 Java 中，<code v-pre>volatile</code> 关键字可以保证变量的可见性，如果我们将变量声明为 <strong><code v-pre>volatile</code></strong> ，这就指示 JVM，这个变量是共享且不稳定的，每次使用它都到主存中进行读取。</p>
<p><strong><code v-pre>volatile</code> 关键字能保证数据的可见性，但不能保证数据的原子性。<code v-pre>synchronized</code> 关键字两者都能保证。</strong></p>
<h3 id="volatile如何禁止指令重排序" tabindex="-1"><a class="header-anchor" href="#volatile如何禁止指令重排序" aria-hidden="true">#</a> volatile如何禁止指令重排序？</h3>
<p>如果将变量通过<code v-pre>volatile</code>修饰，则对这个变量读写的时候会添加上特定的读写屏障保证可见性。</p>
<p>在CPU的物理世界里，内存屏障通常有三种：</p>
<p>lfence: 读屏障（load fence)，即立刻让CPU Cache失效，从内存中读取数据，并装载入Cache中。</p>
<p>sfence: 写屏障（write fence）, 即立刻进行flush，把缓存中的数据刷入内存中。</p>
<p>mfence: 全屏障 (memory fence)，即读写屏障，保证读写都串行化，确保数据都写入内存并清除缓存。</p>
<p>双检查单例模式：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Singleton</span><span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>volatile</code>是“轻量级”<code v-pre>synchronized</code>，保证了共享变量的“可见性”（JMM确保所有线程看到这个变量的值是一致的），当CPU写数据时，如果发现操作的变量是共享变量，即在其他CPU中也存在该变量的副本，<strong>会发出信号通知其他CPU将该变量的缓存行置为无效状态，并且锁住缓存行</strong>，因此当其他CPU需要读取这个变量时，要等锁释放，并发现自己缓存行是无效的，那么它就会从内存重新读取。</p>
<h2 id="乐观锁和悲观锁" tabindex="-1"><a class="header-anchor" href="#乐观锁和悲观锁" aria-hidden="true">#</a> 乐观锁和悲观锁</h2>
...
<h3 id="cas-算法" tabindex="-1"><a class="header-anchor" href="#cas-算法" aria-hidden="true">#</a> CAS 算法</h3>
...
<h3 id="aba问题" tabindex="-1"><a class="header-anchor" href="#aba问题" aria-hidden="true">#</a> ABA问题</h3>
<p>如果有一个线程将变量值A改为B，之后又有一个线程将B改为A，则第三个线程采用CAS进行修改的时候发现预期值A正确，则认为变量没有被修改，其实已经修改了两次。</p>
<p>解决ABA问题就是添加版本号或者是时间戳。</p>
<h3 id="cas问题" tabindex="-1"><a class="header-anchor" href="#cas问题" aria-hidden="true">#</a> CAS问题</h3>
<p>CAS 经常会用到自旋操作来进行重试，也就是不成功就一直循环执行直到成功。如果长时间不成功，会给 CPU 带来非常大的执行开销。</p>
<p>CAS 只对单个共享变量有效，当操作涉及跨多个共享变量时 CAS 无效。但是从 JDK 1.5 开始，提供了<code v-pre>AtomicReference</code>类来保证引用对象之间的原子性，可以把多个变量放在一个对象里来进行 CAS 操作。</p>
<h2 id="synchronized" tabindex="-1"><a class="header-anchor" href="#synchronized" aria-hidden="true">#</a> synchronized</h2>
...
<p>☀️详见<a href="http://ylzhong.top/java/3juc/4synchronizedlock.html" target="_blank" rel="noopener noreferrer">synchronized锁优化<ExternalLinkIcon/></a>。</p>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">synchronized</span><span class="token punctuation">(</span><span class="token keyword">this</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token comment">//锁对象</span>
<span class="token punctuation">}</span>
<span class="token keyword">synchronized</span><span class="token punctuation">(</span>类<span class="token punctuation">.</span><span class="token keyword">class</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token comment">//锁类</span>
<span class="token punctuation">}</span>
...
<p><code v-pre>synchronized</code> 关键字加到 <code v-pre>static</code> 静态方法和 <code v-pre>synchronized(class)</code> 代码块上都是是给 Class 类上锁；</p>
<p><code v-pre>synchronized</code> 关键字加到实例方法上是给对象实例上锁；</p>
<h3 id="synchronized底层原理" tabindex="-1"><a class="header-anchor" href="#synchronized底层原理" aria-hidden="true">#</a> synchronized底层原理</h3>
<p><strong>修饰同步代码块</strong></p>
...
<p><code v-pre>synchronized</code> 修饰的方法并没有 <code v-pre>monitorenter</code> 指令和 <code v-pre>monitorexit</code> 指令，取得代之的是 <code v-pre>ACC_SYNCHRONIZED</code> 标识，该标识指明了该方法是一个同步方法。JVM 通过该 <code v-pre>ACC_SYNCHRONIZED</code> 访问标志来辨别一个方法是否声明为同步方法，从而执行相应的同步调用。</p>
<p><code v-pre>synchronized</code> 同步语句块的实现使用的是 <code v-pre>monitorenter</code> 和 <code v-pre>monitorexit</code> 指令，其中 <code v-pre>monitorenter</code> 指令指向同步代码块的开始位置，<code v-pre>monitorexit</code> 指令则指明同步代码块的结束位置。</p>
<p>两者本质都是对对象监视器<strong>monitor</strong>的获取。</p>
<h3 id="synchronized-和-volatile-有什么区别" tabindex="-1"><a class="header-anchor" href="#synchronized-和-volatile-有什么区别" aria-hidden="true">#</a> synchronized 和 volatile 有什么区别？</h3>
<ul>
<li>
<p><code v-pre>volatile</code> 关键字是线程同步的轻量级实现，所以 <code v-pre>volatile</code>性能肯定比<code v-pre>synchronized</code>关键字要好 。但是 <code v-pre>volatile</code> 关键字只能用于变量而 <code v-pre>synchronized</code> 关键字可以修饰方法以及代码块 。</p>
</li>
<li>
<p><code v-pre>volatile</code> 关键字能保证数据的可见性，但不能保证数据的原子性。<code v-pre>synchronized</code> 关键字两者都能保证。</p>
</li>
<li>
<p><code v-pre>volatile</code>关键字主要用于解决变量在多个线程之间的可见性，而 <code v-pre>synchronized</code> 关键字解决的是多个线程之间访问资源的同步性。</p>
</li>
</ul>
...
<p>☀️详见<a href="http://ylzhong.top/java/3juc/5aqs.html" target="_blank" rel="noopener noreferrer">AQS抽象队列同步器<ExternalLinkIcon/></a>。</p>
...
<h3 id="synchronized-和-reentrantlock-有什么区别" tabindex="-1"><a class="header-anchor" href="#synchronized-和-reentrantlock-有什么区别" aria-hidden="true">#</a> synchronized 和 ReentrantLock 有什么区别？</h3>
<ul>
<li>两者都是可重入锁，也就是线程可以再次获取自己的内部锁。</li>
<li><code v-pre>synchronized</code> 依赖于 JVM 而 <code v-pre>ReentrantLock</code> 依赖于 API。</li>
<li><code v-pre>ReentrantLock</code> 比<code v-pre>synchronized</code> 增加了一些高级功能：
<ul>
<li>等待可中断：<code v-pre>ReentrantLock</code>提供了一种能够中断等待锁的线程的机制，通过 <code v-pre>lock.lockInterruptibly()</code> 来实现这个机制。也就是说正在等待的线程可以选择放弃等待，改为处理其他事情。</li>
<li>可实现公平锁: <code v-pre>ReentrantLock</code>可以指定是公平锁还是非公平锁，而<code v-pre>synchronized</code>只能是非公平锁。</li>
<li>可实现选择性通知（锁可以绑定多个条件）: <code v-pre>synchronized</code>关键字与<code v-pre>wait()</code>和<code v-pre>notify()</code>/<code v-pre>notifyAll()</code>方法相结合可以实现等待/通知机制。<code v-pre>ReentrantLock</code>也可以实现，但是需要借助于<code v-pre>Condition</code>接口与<code v-pre>newCondition()</code>方法。</li>
</ul>
</li>
</ul>
<h3 id="可中断锁和不可中断锁有什么区别" tabindex="-1"><a class="header-anchor" href="#可中断锁和不可中断锁有什么区别" aria-hidden="true">#</a> 可中断锁和不可中断锁有什么区别？</h3>
<ul>
<li><strong>可中断锁</strong>：获取锁的过程中可以被中断，不需要一直等到获取锁之后 才能进行其他逻辑处理。<code v-pre>ReentrantLock</code> 就属于是可中断锁。</li>
<li><strong>不可中断锁</strong>：一旦线程申请了锁，就只能等到拿到锁以后才能进行其他的逻辑处理。 <code v-pre>synchronized</code> 就属于是不可中断锁。</li>
</ul>
</div></template>
