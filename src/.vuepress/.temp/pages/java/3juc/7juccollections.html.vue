...
<p>为了将读取的性能发挥到极致，<code v-pre>CopyOnWriteArrayList</code> 读取是完全不用加锁的，并且更厉害的是：写入也不会阻塞读取操作。<strong>只有写入和写入之间需要进行同步等待</strong>。</p>
<h3 id="copyonwritearraylist-原理" tabindex="-1"><a class="header-anchor" href="#copyonwritearraylist-原理" aria-hidden="true">#</a> CopyOnWriteArrayList 原理</h3>
<p><code v-pre>CopyOnWriteArrayList</code> 类的所有可变操作（add，set 等等）都是通过创建底层数组的新副本来实现的。当 List 需要被修改的时候，我并不修改原有内容，而是对原有数据进行一次复制，将修改的内容写入副本。写完之后，再将修改完的副本替换原来的数据，这样就可以保证写操作不会影响读操作。</p>
<h2 id="concurrentlinkedqueue" tabindex="-1"><a class="header-anchor" href="#concurrentlinkedqueue" aria-hidden="true">#</a> ConcurrentLinkedQueue</h2>
<p>Java 提供的线程安全的 <code v-pre>Queue</code> 可以分为<strong>阻塞队列</strong>和<strong>非阻塞队列</strong>，其中阻塞队列的典型例子是 <code v-pre>BlockingQueue</code>，非阻塞队列的典型例子是 <code v-pre>ConcurrentLinkedQueue</code>，使用链表作为其数据结构。在实际应用中要根据实际需要选用阻塞队列或者非阻塞队列。 <strong>阻塞队列可以通过加锁来实现，非阻塞队列可以通过 CAS 操作实现</strong>。</p>
<p><code v-pre>ConcurrentLinkedQueue</code> 主要使用 CAS 非阻塞算法来实现线程安全。</p>
