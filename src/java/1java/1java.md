...
1. 内部类能很好的隐藏实现，一般非内部类，不允许有 `private` 和 `protected` 权限，但是内部类可以。
2. 内部类拥有外部类所有元素的访问权限。
3. 解决单继承的缺陷，也就是内部类可以自己继承类，实现整个类的多继承。
4. 当想要定义一个回调函数，却不想写大量代码的时候可以通过匿名内部类实现，例如 `new Thread(new Runnable(){...})` 其中就是一个内部类。
