...
- 使用 `stop()` 方法强制中断线程，不推荐。
- 使用 `interrupt()` 方法中断线程，调用方法过后不会立即中断线程，而是设置一个中断标志位，表示这个线程已经被中断，具体行为取决于被中断线程当前状态以及如何响应中断。对于阻塞的线程，例如调用了 `wait()/join()/sleep()` 方法的线程执行这个方法会抛出 `InterruptedException` 并清除中断状态。（调用线程没有执行完之前会一直阻塞在 
  `join()` 方法位置）
