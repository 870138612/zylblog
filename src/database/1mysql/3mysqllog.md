...
如果 `write pos` 追上 `checkpoint` ，表示**日志文件组**满了，这时候不能再写入新的 `redo log` 记录，`MySQL` 得停下来，清空一些记录，把 `checkpoint` 推进一下。(循环链表)

![image-20230529163654789](/markdown/image-20230529163654789.png)
...
- write是吧日志写入page cache，不是数据持久化到磁盘，所以速度比较快。
- fsync超时将数据持久化到磁盘的操作。
...
☀️详见[事务隔离级别和MVCC](http://ylzhong.top/database/1mysql/4mysqlmvcc.html)。
