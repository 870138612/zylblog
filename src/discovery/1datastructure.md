...
- ```java
  int kmp(String zhu, String zi,int[] next) {
      ...
      if (j >= zi.length()) {
          return i - zi.length() + 1;
      }
      return -1;
  }
  ```

- 改进的kmp数组，作用除去了子串的逐步回溯.
...
- 对完全二叉树从上到下，从左到右进行编号，从1开始，则编号为$i$结点的左孩子编号为$2*i$，右孩子$2*i+1$.
...
- 添加了两个属性，`ltag`，`rtag`，`ltag/rtag=0`表示是正常的左子树，是$1$表示是线索.
...
- 堆排序，在完全二叉树中，从上到下，从左到右依次进行编号(从$1$开始)，则$i$号结点的左孩子编号$2*i$，右孩子是$2*i+1$，每次可以确定一个元素的最终位置.
...
- 最佳归并树
    - 最优$n$叉树，在归并的初期，尽量选择较短的归并段进行归并，这样可以减少归并的总次数.
    - 什么情况下最佳归并树是唯一的，当归并段的长度都不相等的时候，最佳归并树是唯一的.
