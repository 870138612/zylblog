...
- 消息抵达 Broker，Broker 要将消息持久化到磁盘中才算成功，假如还没有持久化成功，Broker 宕机了，消息就会丢失，这个时候需要将 publisher 必须确认回调机制，确认成功的消息，修改数据库消息状态。
- 自动 ack 状态下，消费者收到消息，但没来得及确认回复，宕机了，也会导致消息丢失，这个时候需要开启手动 ack，消息成功移除，否则消息就重回队列。
...
producer 端发送消息需要添加异常处理，防止发送途中 MQ 宕机，将发送日志记录，发送失败的消息后续进行重试。
