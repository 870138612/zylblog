...
token 则直接放行，表示这个请求不需要用户验证。有 token 则去 Redis 中寻找，如果没有找到，则表示登录状态过期，需要重新登录。找到了则将对应的用户数据查询并封装放入 `ThreadLocal` 中，用来在这次会话中共享数据，为了防止线程复用导致后续的用户用到了之前数据，则在每次线程执行 `preHandle` 方法时都执行一次 `remove` 将 
`ThreadLocal` 清除数据。
...
将BitMap 和 1 做与运算得到的值就能判断最后一天是否签到，签到则将连续签到值加一，再将 BitMap 右移移位做同样的与操作并统计，知道出现某天没有签到，得到连续签到的天数。
...
> 原文回答：生产者到 broker 的confirm，broker 到消费者的时候手动确认机制，持久化。当然如果 kafka 的话其实可以确认的时候给一个 offset，避免了重复。（这个是面试跟我说的，说主要是想问下根 kafka 的区别。）
...
请求测试通过 **Jmeter** 完成，先通过 CSV 数据文件创建 1000 个用户到数据库，并登录这 1000 个用户，再通过这 1000 个用户模拟秒杀过程。
